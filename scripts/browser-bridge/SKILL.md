...
**Measured** n=1/goal, so read as "≈0.9 correct", **not** a rate. The numbers,
guardrails, and what was NOT measured → `reference/agent.md`.
...
| `activate` | **⚠⚠ TAKES THE OPERATOR'S SCREEN — the ONE intrusive op, a LAST RESORT.** The i3 raise is OPT-IN: `--focus` (auto-on only on a TTY) → raise; else `withheld` — or `skipped` on a host with no i3. It is **NOT** the fix for a hidden/unrendered tab — that is `wake`. Read `reference/spa-wake.md` BEFORE using it |
