...
| `activate` | **⚠⚠ TAKES THE OPERATOR'S SCREEN — the ONE intrusive op, a LAST RESORT.** The i3 raise is consent-gated: `withheld` unless `--focus` (auto-on only when stdout is a TTY). It is **NOT** the fix for a hidden/unrendered tab — that is `wake`. Read `reference/spa-wake.md` BEFORE using it |
