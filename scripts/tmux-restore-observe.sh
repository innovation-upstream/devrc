...
verdict() {
  ...
  if [ -n "$refused" ] && [ "$refused" != 0 ]; then
    ...
    echo "   🔴 THE EXIT CODE DEPENDS ON WHICH REFUSAL IT WAS, so do not read"
    echo "   Result= alone:"
    echo "     * COLD BOOT (nothing had started tmux yet) exits 0 — Result=success,"
    echo "       ExecMainStatus=0 and InactiveExitTimestamp all look EXACTLY like a"
    echo "       successful restore, and this journal line is the only difference."
    echo "     * MID-SESSION (a plan was written during this boot, so a workspace"
    echo "       existed and is gone) exits 75 ONCE per incident, which fires"
    echo "       OnFailure=notify-failure@%n. Later activations for the SAME"
    echo "       incident print the same lines and exit 0 again, because the alert"
    echo "       is latched in \$XDG_RUNTIME_DIR — a tmux socket outlives its"
    echo "       server, so ConditionPathExists= keeps passing and this branch is"
    echo "       reached repeatedly. So ExecMainStatus=0 on a refusal does NOT"
    echo "       mean it was a cold boot; grep the run's own 'cold-boot check:'"
    echo "       and 'alert latch:' lines, which name both."
