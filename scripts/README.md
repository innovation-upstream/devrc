...
| `worktree-prune` | classify git worktrees across many repos as `dead` / `orphan` / `live` / `cannot-tell`, recording the EVIDENCE each verdict rests on. **Dry-run by default**; `--execute` additionally requires `--confirm N` matching the exact removal count, only ever touches `dead` rows, re-checks dirtiness immediately before each removal and never passes `--force`. 🔴 It does **not** decide "did this land" by ancestry: `git merge-base --is-ancestor` is false for every **squash**-merged branch forever, so containment is answered by four independent signals (`ancestor`, `pr-merged` via `gh`, `content-identical` over the branch's changed paths, `patch-equivalent` via `git cherry`) and anything none of them can answer is a loud `cannot-tell`, never a `dead`. 🔴 **`--exclude-path GLOB` (repeatable) and its shorthand `--skip-agent-worktrees` spare individual ROWS**, because repo-level scoping was not fine-grained enough to be safe: a real scan found 860 worktrees / 129 repos → 231 dead and **all 231 removable**, of which **135 were trees under a `.claude/worktrees/` directory belonging to OTHER LIVE Claude sessions** — while excluding whole repos to dodge them also drops the ordinary dead rows those repos hold. With the flag, all 135 are spared and 96 legitimately-dead worktrees stay reachable. An excluded row is still scanned, classified and printed (as `dead (excluded)`, with its own summary line and column), is **never** removed, and is **not** counted toward `--confirm N`. 🔴 **A glob that matches nothing is byte-identical to no glob at all** — and `--confirm N` gives zero independent protection there, because N comes from the same wrong count — so the report ALWAYS echoes the effective glob list with a **per-glob** match count, names any glob matching zero rows, and **`--execute` refuses while any glob is in that state**. Glob semantics: `fnmatch.fnmatchcase` against the absolute path; **`*` crosses `/`**; **subtree** (matching an ANCESTOR excludes the worktree, so naming a directory covers its contents); a trailing `/` is stripped; the symlink-resolved path is tested too; a literal path containing `* ? [` needs `glob.escape` (note: `fnmatch` has no `escape`). `--skip-agent-worktrees` is `*/.claude/worktrees/*`, **not** `…/agent-*` — 1 of 246 such entries on this box has no `agent-` prefix and is a live session's tree |
