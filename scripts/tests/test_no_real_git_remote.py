...
SCOPE, stated so the title cannot be read as more than it is:
  * This covers what `testlib/gitwrite_plugin.py` is loaded into, which is
...
from testlib import gitwrite  # noqa: E402
from testlib.mockbin import write_exec  # noqa: E402
from testlib import gitwrite_plugin  # noqa: E402
...
def _install(stub: Path, **kw):
    """`gitwrite.install`, but resolving the REAL git and dodging L2.

    🔴 A test that calls `gitwrite.install()` naively gets a shim whose baked
    ...
    try:
        with _l2_off():
            return gitwrite.install(stub, **kw)
    finally:
        if saved is None:
...
def _handshake(stub: Path):
    """`gitwrite.handshake`, with L2 out of the way.
    ...
    """
    with _l2_off():
        return gitwrite.handshake(stub)


def _real_path() -> str:
    """PATH with the shim directory removed — the ambient, unprotected one."""
    stub = os.environ.get(gitwrite.STUB_DIR_ENV, "")
    farm = str(gitwrite.exec_path_dir(stub)) if stub else ""
...
def _env(*, allowed: str, denied: str, path: str | None = None) -> dict:
    e = dict(os.environ)
    e[gitwrite.ROOTS_ENV] = allowed
    e[gitwrite.DENY_ROOTS_ENV] = denied
    if path is not None:
        e["PATH"] = path
...
def _expect_closed(tmp_path: Path, builder, *, name: str) -> None:
    ...
    assert unguarded.returncode != gitwrite.BLOCK_EXIT, (
        f"{name}: the UNGUARDED control was itself refused by this policy "
        f"(rc={gitwrite.BLOCK_EXIT}). The control is not unguarded — see "
...
def test_the_exec_path_farm_is_not_itself_on_path(no_real_git_writes):
    ...
    """
    farm = str(gitwrite.exec_path_dir(no_real_git_writes))
    entries = [p for p in os.environ["PATH"].split(os.pathsep) if p]
    ...
    assert os.environ.get("GIT_EXEC_PATH") == farm, (
        ...
        "resolves `git` to the real binary — see gitwrite.exec_path_farm()")
...
def test_the_session_denies_the_tree_under_test(no_real_git_writes):
    ...
    denied = os.environ[gitwrite.DENY_ROOTS_ENV].split(":")
    assert str(gitwrite.repo_root()) in denied, (
        f"the tree under test ({gitwrite.repo_root()}) is not in "
        f"{gitwrite.DENY_ROOTS_ENV}={denied!r}. In the nix sandbox the source is "
...
def test_the_read_verb_ledger_is_pinned():
    ...
    """
    assert set(gitwrite.GIT_READ_VERBS) == {
...
def test_the_shim_has_no_env_readable_mode(tmp_path):
    ...
    the live one.
    """
    body = gitwrite.git_body("/bin/true", Path("/dev/null"), stub_dir=tmp_path)
    assert gitwrite.MODE_ENV not in body, (
        f"the shim still reads {gitwrite.MODE_ENV} from the environment. Mode is "
...
def test_the_env_ledger_no_longer_holds_the_known_non_vectors():
    ...
    for bad in ("GIT_NAMESPACE", "GIT_CEILING_DIRECTORIES",
        ...
        assert bad not in gitwrite.GIT_LOCATION_ENV, (
...
@pytest.mark.parametrize("var", sorted(gitwrite.GIT_LOCATION_ENV))
...
_ENV_VECTORS = {
    ...
    "GIT_COMMON_DIR": (".git", ("config", "--local", "gitwrite.pwned", "1"),
                       None),
...
def test_the_env_vector_table_covers_the_ledger_exactly():
    ...
    """
    assert set(_ENV_VECTORS) == set(gitwrite.GIT_LOCATION_ENV), (
        "the live-vector table and GIT_LOCATION_ENV disagree: "
        f"ledger-only={sorted(set(gitwrite.GIT_LOCATION_ENV) - set(_ENV_VECTORS))}, "
        f"table-only={sorted(set(_ENV_VECTORS) - set(gitwrite.GIT_LOCATION_ENV))}")
...
def test_a_git_with_no_verb_prints_usage_instead_of_refusing(
    ...
    assert gitwrite.BANNER not in got.stderr
...
def _b_audit_mode(root, victim, allowed):
    return (["-C", str(victim), "commit", "--allow-empty", "-m", "AUDITPWN"],
            root, {gitwrite.MODE_ENV: "audit"}, None)
...
def test_an_audit_mode_env_var_cannot_disarm_the_guard(
    ...
    victim = _victim(tmp_path / "victim")
    env = _env(allowed=str(tmp_path / "nowhere"), denied=str(victim))
    env[gitwrite.MODE_ENV] = gitwrite.MODE_AUDIT
    before = _victim_state(victim)
    got = _git("commit", "--allow-empty", "-m", "AUDITPWN", cwd=victim,
               env=env)
    assert got.returncode == gitwrite.BLOCK_EXIT, (
        f"{gitwrite.MODE_ENV}={gitwrite.MODE_AUDIT} disarmed the guard "
        f"(rc={got.returncode})")
...
def test_audit_mode_still_exists_as_a_bake_time_argument(tmp_path):
    ...
    _install(stub, allowed=str(tmp_path / "nowhere"),
                  denied=str(tmp_path), mode=gitwrite.MODE_AUDIT)
...
def test_a_shell_alias_writing_the_victim_is_NOT_covered(
    ...
    assert marker.exists(), (
        ...
        "'stated limit' paragraphs in testlib/gitwrite.py and "
        "testlib/gitwrite_plugin.py, which currently tell the reader it is not "
        "covered.")
...
def test_the_policy_survives_a_child_with_a_REPLACING_env(
    ...
    """
    stub = Path(os.environ[gitwrite.STUB_DIR_ENV])
    ...
    assert gitwrite.ROOTS_ENV not in bare and gitwrite.DENY_ROOTS_ENV not in bare
    ...
    ok = subprocess.run(["git", "init", "-q", str(fixture_parent / "r")],
                        ...
                        text=True, timeout=TIMEOUT)
    assert ok.returncode != gitwrite.BLOCK_EXIT, (
    ...
    bad = subprocess.run(
        ["git", "-C", str(gitwrite.repo_root()), "config", "gitwrite.probe", "1"],
        cwd=str(tmp_path), env=bare, capture_output=True, text=True,
        timeout=TIMEOUT)
    assert bad.returncode == gitwrite.BLOCK_EXIT, (
...
def test_the_exec_path_farm_survives_a_child_with_a_REPLACING_env(
    ...
    bare = {"HOME": str(root / "home"), "PATH": os.environ["PATH"],
            gitwrite.ROOTS_ENV: str(allowed),
            gitwrite.DENY_ROOTS_ENV: str(victim)}
...
def test_the_deny_floor_is_a_union_not_an_override(tmp_path, no_real_git_writes):
    ...
    body = gitwrite.git_body("/bin/true", Path("/dev/null"),
                          allowed_roots="/BAKED_ALLOW",
                          denied_roots="/BAKED_DENY", stub_dir=tmp_path)
    assert f'roots="${{{gitwrite.ROOTS_ENV}:-/BAKED_ALLOW}}"' in body
    assert (f'denied="/BAKED_DENY${{{gitwrite.DENY_ROOTS_ENV}:+'
            f':${{{gitwrite.DENY_ROOTS_ENV}}}}}"') in body, (
        "the deny floor is not a union — a caller supplying "
        f"{gitwrite.DENY_ROOTS_ENV} could drop the tree under test from it")
...
def test_the_deny_floor_is_a_union_in_practice(tmp_path, no_real_git_writes):
    ...
    env[gitwrite.DENY_ROOTS_ENV] = str(tmp_path / "somewhere-else")
    env.pop(gitwrite.ROOTS_ENV, None)
    before = _victim_state(victim)
    with _l2_off():
...
def test_the_session_scrubs_an_inherited_git_env(no_real_git_writes):
    ...
    for var in (*gitwrite.GIT_LOCATION_ENV, *gitwrite.GIT_EXEC_ENV,
                gitwrite.MODE_ENV, "GIT_CONFIG_COUNT"):
...
def test_the_same_write_succeeds_with_the_shim_removed(
    ...
    guarded = subprocess.run(
        ...
        env=_env(allowed=str(tmp_path / "nowhere"), denied=str(victim)),
        capture_output=True, text=True, timeout=TIMEOUT)
    assert guarded.returncode == gitwrite.BLOCK_EXIT
...
def test_the_handshake_accepts_this_policys_own_shim(tmp_path):
    stub = tmp_path / "stub"
    _install(stub, allowed=str(tmp_path), denied=str(gitwrite.repo_root()))
    answer = _handshake(stub)
    assert answer is not None
    assert answer["fingerprint"] == gitwrite.policy_fingerprint()
...
def test_the_handshake_rejects_anything_that_is_not_this_policys_shim(
    ...
    if flavour == "passthrough":
        ...
        write_exec(
            exe,
            f'printf "{gitwrite.HANDSHAKE_MAGIC} deadbeefdeadbeef\\n"\n'
            'printf "roots=/\\n"\nprintf "denied=/\\n"\nexit 0\n')
...
def test_a_foreign_inherited_shim_FAILS_the_session_instead_of_being_adopted(
    ...
    exe = d / "git"
    write_exec(exe, f'exec {_real()} "$@"\n')
    monkeypatch.setenv(gitwrite.STUB_DIR_ENV, str(d))
    with _l2_off(), pytest.raises(gitwrite_plugin.ForeignGitShim) as exc:
        gitwrite_plugin._inherited_stub_dir()
...
def test_an_inherited_shim_scoped_to_another_tree_is_REPLACED_not_adopted(
    ...
    if flaw == "wrong-deny-floor":
        ...
        _install(stub, allowed=str(tmp_path), denied=str(gitwrite.repo_root()),
                 mode=gitwrite.MODE_AUDIT)
    assert _handshake(stub) is not None, "the shim IS this policy's"
    monkeypatch.setenv(gitwrite.STUB_DIR_ENV, str(stub))
    with _l2_off():
        assert gitwrite_plugin._inherited_stub_dir() is None, (
...
def test_the_fingerprint_changes_when_the_policy_changes(monkeypatch):
    ...
    """
    before = gitwrite.policy_fingerprint()
    monkeypatch.setattr(gitwrite, "GIT_LOCATION_ENV",
                        (*gitwrite.GIT_LOCATION_ENV, "GIT_MADE_UP_FOR_THE_TEST"))
    after = gitwrite.policy_fingerprint()
    assert after != before, (
...
def test_an_absolute_path_git_write_is_intercepted_in_process(
    ...
    got = subprocess.run([_real(), "-C", str(victim), "config",
                          "gitwrite.absprobe", "1"], cwd=str(tmp_path), env=env,
                         capture_output=True, text=True, timeout=TIMEOUT)
    assert got.returncode == gitwrite.BLOCK_EXIT, (
...
def test_the_absolute_path_redirect_is_narrow(argv, expect_redirect, tmp_path):
    ...
    got = gitwrite_plugin._redirect_argv(argv, tmp_path)
...
def test_the_shim_dir_and_the_farm_are_not_redirected(tmp_path):
    ...
    stub = tmp_path / "stub"
    _install(stub, allowed=str(tmp_path), denied=str(tmp_path / "nope"))
    assert gitwrite_plugin._redirect_argv([str(stub / "git"), "status"],
                                       stub) is None
    assert gitwrite_plugin._redirect_argv(
        [str(gitwrite.exec_path_dir(stub) / "git"), "status"], stub) is None
...
def test_the_popen_patch_composes_in_EITHER_order(tmp_path):
    ...
    try:
        for label, first, second in (
                ("nolaunch then gitwrite",
                 nolaunch_plugin._patch_subprocess,
                 gitwrite_plugin._patch_subprocess),
                ("gitwrite then nolaunch",
                 gitwrite_plugin._patch_subprocess,
                 nolaunch_plugin._patch_subprocess)):
            subprocess.Popen = saved
            first(a)
...
def test_protected_without_asking():
    ...
    assert Path(resolved).parent == Path(os.environ[gitwrite.STUB_DIR_ENV]), (
...
def test_the_session_marker_was_written_and_says_how(no_real_git_writes):
    ...
    """
    lines = gitwrite.recorded(no_real_git_writes)
    markers = [ln for ln in lines
               if ln.startswith(gitwrite_plugin.SESSION_MARKER)]
    assert markers, (
        "no session marker in the shim log — this plugin did not load, and a "
        "clean log would be indistinguishable from a clean run")
    assert any(f"via={gitwrite_plugin.VIA_SELF}" in ln
               or f"via={gitwrite_plugin.VIA_INHERITED}" in ln
               for ln in markers), (
