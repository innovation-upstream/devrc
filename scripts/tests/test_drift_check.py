...
def test_the_timer_is_gated_by_the_master_switch_and_the_service_is_not():
    ...
    # switch must ship OFF, enabling a timer is a live change to a host and
    # belongs in its own supervised deploy". #406 IS that supervised deploy: it
    # measured both preconditions the switch's own comment demanded (the ssh leg
    # reaching the laptop from a systemd-user context, which found genuine drift
    # on its first hand-run; and the failure toast actually DISPLAYING, which was
    # FALSE at first — dunst was paused with 286 queued, so the toast exited 0
    # and was binned) and then flipped it to true.
    #
    # So the assertion was retired by the thing it was guarding against, and the
    # value is now a deployment decision rather than an invariant. What remains
    # an invariant is the GATING above: the timer's WantedBy must stay behind the
    # switch, and the service must stay unconditional, whichever way the switch
    # is set. Pinning the value here again would only re-break the gate the next
    # time someone legitimately changes it.
    assert "enableDriftDeadman" in HOME_NIX, (
        "the master switch binding is gone, so the gating expression above "
