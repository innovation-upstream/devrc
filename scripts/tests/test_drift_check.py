...
class Fleet:
    ...
    def check(self, *args, repo=None, script=None, **envextra):
        env = self.env(
            ...
            # ⚠ `DRIFT_SESSION_MANAGER` WAS PINNED HERE AND IS NOW GONE WITH THE
            # THING IT DEFENDED. It was the FOURTH hermeticity seam: the
            # fuzzyclaw phase-2 gate exec'd `scripts/session-manager`, which
            # scans the operator's REAL tmux — unstubbed, every test in this file
            # did exactly that (48 live windows, measured) and one declared
            # "READY — 0 of 48" because the fixture $HOME has no task files. A
            # read-only breach is still a breach, and the verdict was wrong.
            #
            # The gate is retired, drift-check.sh execs no child and reads no
            # such variable, so there is nothing left to point at a stub. Setting
            # it would be pinning a name the subject under test does not have.
            # The LESSON is not retired — it is why the `gh` seam below exists.
...
def test_a_plain_clean_run_does_not_gain_the_verdict_block(fleet):
    """A clean run prints the affirmative line and NO verdict block.

    🔴 KEPT AND RE-HOMED when the phase-2 gate was retired. The affirmative
    branch used to read `rc = 0 || rc = 16` so the ACTIONABLE-not-drift code
    could print BOTH the cleanup notice and "no drift on the host(s) CHECKED";
    this test was its other half, pinning that widening the branch did not give
    an ORDINARY rc-0 run a verdict line it had not earned. The gate is gone and
    the guard is back to `rc = 0`, which is exactly when a guard against
    accidental widening is easiest to drop and most useful to keep.
    """
    fleet.catch_up()
    rc, out = fleet.check("--no-remote")
    assert rc == 0, out
...
def test_a_real_drift_verdict_does_not_gain_the_no_drift_line(fleet):
    ...
    🔴 KEPT for the same reason as the test above, and it is the one that would
    catch the removal going too far: narrowing `rc = 0 || rc = 16` to `rc = 0`
    is correct, but the same edit made carelessly (dropping the guard, or
    inverting it) would let a genuine drift verdict print "no drift on the
    host(s) CHECKED" beside it.
    """
    fleet.catch_up()
    fleet.add_local_commit("commit the workbench never pushed")
    rc, out = fleet.check("--no-remote")
...
def test_NO_exit_code_is_excused_from_failing_the_unit():
    """🔴 THE EXCUSE LIST IS EMPTY, AND THAT IS AN ASSERTION, NOT AN ABSENCE.

    This replaces `test_only_16_is_excused_from_failing_the_unit`. That guard
    existed because `SuccessExitStatus` takes a LIST: a second entry beside 16
    would silently mute a real drift verdict, which is the deadman's whole
    purpose defeated. rc 16 is retired, so the correct excuse list is now NO
    list at all — and simply deleting the old guard would leave the widening
    hazard completely unguarded, which is strictly worse than what it replaced.

    So the claim inverts rather than disappearing: the drift-check service block
    must carry no `SuccessExitStatus` whatsoever. Every code the script can
    still return is a real finding that SHOULD reach OnFailure.

    An INVARIANT GUARD, labelled as such: it is green at the base sha for the
    OTHER reason (16 was excused there, so `== ["16"]` held), and it fires the
    moment somebody re-adds an excuse for any code.

    🔴 IT ASSERTS THE SETTING, NOT THE WORD, and the first draft got that wrong.
    A bare `"SuccessExitStatus" not in block` is a SPELLED guard: it went red on
    the nix COMMENT that explains why the setting is absent, so the only way to
    make it pass would have been to stop explaining. Comment lines are stripped
    first, so what is pinned is an ASSIGNMENT — the thing systemd reads.
    """
    block = _drift_service_block()
    code = "\n".join(
        ln for ln in block.splitlines() if not ln.strip().startswith("#"))
    m = re.search(r"SuccessExitStatus\s*=", code)
    assert not m, (
        "the drift-check unit excuses an exit code from failing: "
        f"{re.search(r'SuccessExitStatus[^;]*;', code).group(0)!r}. Every code "
        "this script can return is a real finding and must reach OnFailure.")
    # POSITIVE CONTROL, in two directions. (a) the block really is the
    # drift-check service and really was read — an empty or renamed block would
    # satisfy the assertion above by containing nothing at all; (b) the comment
    # stripper did not eat the whole block, which would do the same thing.
    assert 'Type = "oneshot"' in code, code[:400]
    assert "drift-check.sh" in code, code[:400]
    # ...and the stripper is doing its job: the rationale comment IS present in
    # the raw block and IS gone from the stripped one, so the two are different
    # texts and this test is reading the one it means to.
    assert "SuccessExitStatus" in block, (
        "the rationale comment for the absent setting is gone — if the setting "
        "is ever re-added, nothing will explain why it must not be")
...
UNIT_PATH_REQUIREMENTS = {
    ...
    "bash": "pkgs.bash",
    # `timeout` caps every capped subprocess this script runs — the source-repo
    # fetch (rc 17) and the branch-protection probes (rc 24) — and `python3`
    # runs `lib/skill_tier_facts.py` for the skill-tier arm (rc 22). Both are
    # resolved from PATH BY THIS SCRIPT, which is what puts them in this table.
    # ⚠ Both ALSO served the retired fuzzyclaw phase-2 gate; that gate going
    # away did not make either dead, which is why they are still here and
    # `tmux` — which ONLY that gate's child needed — is not.
...
def test_a_non_integer_source_fetch_timeout_is_rejected(fleet):
    ...
    line. Its sibling for the retired `DRIFT_PHASE2_TIMEOUT` survived a mutation
    sweep because nothing exercised it; this is the same guard, exercised."""
...
def test_the_unit_start_timeout_can_absorb_every_source_repo_fetch():
    ...
    cap = int(m.group(1))
    # 🔴 THE PHASE-2 SCAN TERM IS GONE FROM THIS MODEL because the leg is gone.
    # It contributed a 60s `DRIFT_PHASE2_TIMEOUT` cap; that gate is retired, the
    # tunable no longer exists, and reading it would fail on its absence rather
    # than on a real budget breach. Dropping a term from a TIMEOUT budget makes
    # the model STRICTER (less reserved slack), so this cannot mask a ceiling
    # that is too low — the direction that matters here.
    phase2 = 0
...
def test_the_ceiling_clears_every_DEFAULT_this_script_sets():
    ...
    default containing a digit — the since-retired DRIFT_PHASE2_TIMEOUT, at 60,
    the LARGEST of them and therefore the only one a ceiling of 50 would have
    caught. (Both ledgers are DERIVED, so its removal needed no edit here — which
    is the property this two-way pin exists to buy.) A
...
def test_a_ZERO_accepting_tunable_still_accepts_its_zero(fleet):
    ...
    fleet.seed_nix_read()
    # ⚠ `DRIFT_PHASE2_TIMEOUT` was the first entry here and is REMOVED, not
    # left in place: the script no longer reads it, so asserting rc 0 while
    # passing it would be a dead element reading as coverage — the run would go
    # green for a variable nothing consults.
    for var in ("DRIFT_SRC_FETCH_TIMEOUT",
