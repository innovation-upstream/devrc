...
WHAT IS UNDER TEST (see scripts/opencode/README.md for the measurements):
  ...
  3. The resolver model itself. opencode 1.18.30 resolves a permission by
...
# 🔴 the resolver model
#
# Faithful port of opencode 1.18.30's `Wildcard.match` + the `findLast` resolver.
...
def test_no_nonexistent_tools_in_permission_block(dead):
    """There is no `list` and no `websearch` tool on 1.18.30. The resolved tool
    ...
    assert dead not in load_config()["permission"], (
        f"{dead!r} is not a tool on opencode 1.18.30 — the key does nothing"
    )
...
def test_nav_is_kept_lean():
    ...
    VERIFIED against `opencode debug agent nav` on 1.18.30: with these denies the
...
def test_no_agent_promises_a_list_tool():
    """There is NO `list` tool on opencode 1.18.30."""
