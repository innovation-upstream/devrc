...
ACKNOWLEDGED_UNSTUBBED = {
    "nvim-octo": (
        ...
        "🔴 THE SCAN IS TEXT-BASED AND THAT IS WHY THIS ROW EXISTS RATHER THAN "
        "A REWORDING. Deleting the token from those comments would clear the "
        "finding while changing nothing about the code — the exact 'a guard on "
        "WORDS is walkable by REWORDING' move RULES.md forbids — and it would "
        "also destroy the only in-file record of why the spawn looks the way it "
        "does. An acknowledgement that says 'prose' is honest; a comment edited "
        "to dodge a grep is not. "
        "🔴 THE PIN that keeps this row from blinding anything: "
        "test_mention_open.py::test_mention_open_SPAWNS_these_argv0_AND_"
        "NOTHING_ELSE asserts the handler's spawn argv[0] set EXACTLY and "
    ...
    "mention-review": (
        ...
        "names the string as a PATH and a PACKAGE, never as a command: "
        "`GO_MODULE=\"nix/pkgs/tools/mention-review/src\"` and the per-package "
        "test-count floor row `cmd/mention-review|4`. ⚠ Stated precisely "
        "because it is nearly a real one — the tests under `cmd/mention-review` "
        "DO exercise the argv contract against a binary, but it is one `go "
        "test` compiles into its own sandbox, not the host binary this ledger "
        "is about. No `mention-review` on PATH is reached by that script. "
        "The `mention-open.py` half is the spawn, and it is: "
        ...
        "--class float,mention-review … -e mention-review <owner/repo> "
        "<number>` — and `alacritty` IS in HOST_LAUNCHERS, so the suite's stub "
        "intercepts the parent and the payload is never exec'd at all. "
        "⚠ THIS ROW WAS `nvim-octo` UNTIL THE CLICK PATH WAS FLIPPED; it moves "
        "with `REVIEW_EXE`, and octo is now spawned by nothing. "
        "Stubbing it "
        "instead would be actively WRONG, and not merely redundant: the stub "
        "dir is on PATH, so a `mention-review` stub would make `shutil.which` "
        ...
        "🔴 THAT PARAGRAPH PREDICTED A REAL FAILURE AND THE MECHANISM WAS NOT "
        "A STUB. `mention-review` is in `home.packages` so the operator can run "
        "it by hand, so it is on the dev host's REAL PATH — flipping "
        "`REVIEW_EXE` to it turned 22 tests in test_mention_open.py red at "
        "once (measured 2026-09-16), exactly the silent move described above "
        "but caused by an ordinary install rather than by the suite. The file "
        "now pins the lookup itself in an autouse fixture "
        "(`_the_review_tui_is_NOT_on_the_hosts_PATH`), so the invariant its "
        "section comment always CLAIMED is now enforced instead of inherited "
        "from what happens to be installed. ⚠ The two tiers disagreed on this: "
        "the `nix build` sandbox has no such binary, so CI would have stayed "
        "GREEN while the dev host was red. "
        ...
        "`subprocess.Popen(['mention-review', …])` added here fails THAT test "
        ...
        "`pkgs.mention-review` on the hint wrapper's PATH, in BOTH directions — so "
    ...
    "home-manager": (
        ...
        "i3status-scratchpads (added 2026-09-11 with the bar legend's socket "
        "search) is the same PROSE-MENTION shape, and the words are the "
        "FINDING rather than decoration: its occurrences are in the comment "
        "above `_socket_roots` recording that `TMUX_TMPDIR=/run/user/1000` "
        "appears in no /etc/nixos file, no environment.d and no home-manager "
        "output — the same undeclared-runtime-state gap nix/home.nix records at "
        "length on its tmux-restore path unit, and the reason this block "
        "FINDS the socket instead of inheriting the answer. "
        "⚠ THAT SEARCH IS DEFENCE AGAINST A STATE NOT CURRENTLY OBSERVED, "
        "and this justification said otherwise for one round: it claimed the "
        "shipped script rendered `scratch ?` while scratchpads were live. That "
        "measurement was taken with `TMUX_TMPDIR` UNSET, which is not the "
        "environment the bar runs in — MEASURED 2026-09-11 on the LIVE "
        "`i3status-rs` process, /proc/<pid>/environ carries "
        "`TMUX_TMPDIR=/run/user/1000`, and under exactly that environment a "
        "bare `tmux list-sessions` answers correctly. No live bug was "
        "demonstrated. What is established: the variable is undeclared runtime "
        "state whose source is unknown, and both failure modes (a persistent "
        "`?`, and a false zero via a stale /tmp socket) are real and reachable "
        "if it ever goes missing. Deleting the word would delete the "
        ...
        "tmux-scratch-picker.sh (added 2026-09-11, round 2) is the same "
        "PROSE-MENTION shape and is re-justified, not reworded. Its single "
        "occurrence is one clause of the comment above the fzf EXIT-CODE "
        "branch, recording the NON-HYPOTHETICAL way a bare-name `fzf` fails to "
        "launch: a `home-manager switch` writes two profile generations and "
        "the intermediate one drops every `home.packages` binary for ~1 s, so "
        "anything invoked by bare command name during a switch dies "
        "\"command not found\". An fzf that never ran yields the SAME empty "
        "selection as a deliberate dismissal, and exiting 0 on it makes the "
        "float terminal vanish — the defect class the hold exists for. Naming "
        "the mechanism is the whole point of the sentence. 🔴 THE PIN: "
        "test_scratchpads_block.py::test_the_picker_SPAWNS_these_AND_NOTHING_"
        "ELSE runs the script in a PATH JAIL containing only "
        "{tmux, fzf, grep, sort, id, date} across all SIX of its branches and "
        "asserts BOTH directions — nothing outside the set is reached (a "
        "`command not found` in stderr would name it) and none of the set has "
        "silently gone. It also asserts that the COMMENT-STRIPPED source "
        "contains none of home-manager / nixos-rebuild / systemctl / i3-msg / "
