...
class TestOwnershipIsStructuralNotASubstring:
    ...
    def test_a_foreign_hook_quoting_the_marker_is_not_uninstalled(self, repo):
        """🔴 The marker is quoted MID-LINE and the hook also carries an `impl=`
        line, so BOTH halves of the ownership test are exercised. An earlier
        fixture wrote the marker at column 0, which meant the anchoring half was
        unguarded: un-anchoring the grep left the suite green."""
        hooks = self._hooks(repo)
        hooks.mkdir(parents=True, exist_ok=True)
        foreign = hooks / "prepare-commit-msg"
