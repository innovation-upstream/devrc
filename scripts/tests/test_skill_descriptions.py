...
def test_the_listing_total_does_not_regrow_past_its_ratchet():
    ...
    assert total <= LISTING_TOTAL_CEILING_CHARS, (
        ...
        "This loads on EVERY session. On OVERFLOW Claude Code drops descriptions "
        "starting with the skills invoked LEAST, taking their trigger keywords "
        "with them, with no error.\n"
        "🔴 BUT THIS RATCHET IS NOT THE OVERFLOW POINT, AND YOU ARE "
        "PROBABLY NOT OVERFLOWING. The real budget is "
        "`contextWindow * zx(model) * skillListingBudgetFraction` CHARACTERS -- "
        "no tokenizer -- where zx is 4 up to model 4.6 and 3 for claude-opus-5+, "
        "i.e. 6,000 at 200k and 30,000 at 1M. Measured 2026-08-25 the WHOLE "
        "listing sat at ~0.67x of budget on 1M. This constant is a RATCHET on "
        "the one number devrc controls, not a cliff. Do not panic-delete.\n"
        "\nEVICTION PLAYBOOK -- do these in order, in THIS commit:\n"
        "  0. DEMOTE, DON'T DELETE. `claude/skill-tiers.json` (see "
        "scripts/tests/test_skill_tiers.py) moves a skill to tier B / "
        "`name-only`, which costs `len(name) + 2` chars -- about 12 instead of "
        "about 350 -- while the skill stays installed, fully functional and "
        "/name-invocable. It loses only the routing prose that makes it fire "
        "from a described symptom, so demote what Zach reaches for BY NAME. "
        "Verified live 2026-08-24. This is almost always the right move and it "
        "is one line; retiring a skill is not the cheap option it looks like -- "
        "the three retired in #785 bought 772 chars, about 4.5 DAYS of growth at "
        "the measured ~15 new skills/month.\n"
        ...
        "  3. If you are ADDING a skill, pay for it -- by DEMOTING one to tier B "
        "(step 0) before considering retirement, since demotion keeps the "
        "capability and retirement does not. Rank "
