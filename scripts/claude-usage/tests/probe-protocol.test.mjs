...
// --- the double-probe gate -------------------------------------------------- //
//
// 🔴 ONE PAGE LOAD RAN THE WHOLE PROBE TWICE, so every claude.ai open cost
// two org fetches plus two usage fetches per org where one of each would do.
// autoRun() fires at document_idle; the worker's onTabUpdated independently
// asks for a probe at status "complete". The worker's PROBE_MIN_INTERVAL_MS
// gates its ask against OTHER asks and cannot see the auto-run at all — the
// two live in different contexts — so the gate has to sit where they converge.

test("a cu:probe arriving right after the auto-run is skipped", () => {
  const P = globalThis.__CU_PROBE__;
  const now = 1_000_000;
  P._setLastRunAt(now);
  assert.equal(P.tooSoon(now + 1), true, "same turn");
  assert.equal(P.tooSoon(now + P.MIN_RUN_GAP_MS - 1), true, "just inside the window");
});

test("the 15-minute alarm re-probe is NOT caught by that gate", () => {
  // The case to check first if MIN_RUN_GAP_MS is ever raised: the worker's
  // periodic re-probe must keep working, or the extension stops updating
  // while a long-lived tab sits open.
  const P = globalThis.__CU_PROBE__;
  const now = 1_000_000;
  P._setLastRunAt(now);
  assert.equal(P.tooSoon(now + P.MIN_RUN_GAP_MS), false, "exactly at the boundary");
  assert.equal(P.tooSoon(now + 15 * 60 * 1000), false, "the alarm cadence");
});

test("a probe that has never run is never 'too soon'", () => {
  const P = globalThis.__CU_PROBE__;
  P._setLastRunAt(0);
  assert.equal(P.tooSoon(0), false);
  assert.equal(P.tooSoon(Date.now()), false);
});

test("startProbe stamps the gate BEFORE awaiting, not after", async () => {
  // Stamping after the fetch would leave the entire request window unguarded
  // — and that window is exactly where the second caller lands.
  const P = globalThis.__CU_PROBE__;
  P._setLastRunAt(0);
  const t = 5_000_000;
  const running = P.startProbe(t);
  assert.equal(P._lastRunAt(), t, "gate not stamped synchronously");
  await running;
});
