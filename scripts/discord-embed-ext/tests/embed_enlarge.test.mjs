...
import { parseHTML } from "./fake_discord_dom.mjs";
...
test("the module exposes its pure functions and starts nothing when autostart suppressed", () => {
  ...
  assert.equal(typeof DEE.extractChannelId, "function");
...
test("extractChannelId parses channel ID from Discord URL", () => {
  assert.equal(DEE.extractChannelId("https://discord.com/channels/@me/123456789/987654321"), "123456789");
  assert.equal(DEE.extractChannelId("https://discord.com/channels/@me/999/111"), "999");
});

test("extractChannelId returns null for non-channel URLs", () => {
  assert.equal(DEE.extractChannelId("https://discord.com/channels"), null);
  assert.equal(DEE.extractChannelId("https://example.com"), null);
  assert.equal(DEE.extractChannelId(null), null);
  assert.equal(DEE.extractChannelId(""), null);
});
