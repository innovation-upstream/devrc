(function () {
  ...
  // ONE definition, in embed_enlarge.js, for the same reason as isMedia above.
  // unclipAncestors needs this exact boundary too, and a Discord CSS reshuffle
  // that rots the heuristic must not fix one caller and leave the other.
  function messageContainerOf(el) {
    var dee = (typeof globalThis !== "undefined") && globalThis.__DEE__;
    if (!dee || typeof dee.findMessageContainer !== "function") return null;
    return dee.findMessageContainer(el);
  }
  ...
  function getMediaSiblings(mediaEl) {
    var container = messageContainerOf(mediaEl);
