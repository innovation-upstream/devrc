#!/usr/bin/env python3
"""Serve the generated explainer page on the workbench LAN. STATIC, and loud
about its own age.
...
recognise, so there is no `<body>` to inject after — the stale bytes are NOT
served. The response is the interstitial instead. There is deliberately no code
path in this file that emits stale artefact bytes without the banner; that is
the property `scripts/tests/test_present_serve.py` pins, and it is the property
a naive `SimpleHTTPRequestHandler` would not have.
...
def stale_banner(age_seconds: float, artefact: str) -> str:
    """The banner injected into a stale page.
...
def inject_banner(page: str, age_seconds: float, artefact: str) -> str | None:
    """Return `page` with the stale banner injected, or None if it cannot be.
    ...
    if i < 0:
        return None
    at = i + len(_BODY_OPEN)
    return page[:at] + stale_banner(age_seconds, artefact) + page[at:]
...
def _unbannerable_page(artefact: str, age_seconds: float) -> str:
    return _shell(
        "A stale page was withheld", "#8b1a1a",
        f"<p><code>{html.escape(artefact)}</code> is "
        f"{html.escape(humanise(age_seconds))} old, and this server could not "
        "inject its staleness banner into it &mdash; the document does not "
        "carry the <code>&lt;body&gt;</code> opening tag the injector matches "
        "on.</p>"
        "<p>So it was NOT served. A stale page without its banner is "
        "indistinguishable from a current one, which is the single outcome this "
        "server exists to prevent; withholding it is the honest degradation.</p>"
...
def build_response(directory: Path, path: str, *, stale_after: float,
    ...
    two of them: fresh-verbatim and stale-with-banner. That is the property
    worth reading this function for.
    """
    ...
    hdr = {
        ...
        # Machine-readable twin of the banner, off the same boolean.
        "X-Present-Stale": "1" if stale else "0",
    }
    if not stale:
        hdr["X-Present-State"] = "fresh"
        return (200, "text/html; charset=utf-8", hdr, page.encode("utf-8"))

    bannered = inject_banner(page, age, name)
    if bannered is None:
        hdr["X-Present-State"] = "stale-withheld"
        return (503, "text/html; charset=utf-8", hdr,
                _unbannerable_page(name, age).encode("utf-8"))
    hdr["X-Present-State"] = "stale-bannered"
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        prog="present-serve",
        description="Serve the generated devrc explainer page (static, LAN-only).")
    ...
    try:
        ...
        print("present-serve:   the host must be an address THIS machine holds "
              "(the workbench's own eth1 LAN IP), never a homelab node.",
              file=sys.stderr)
        return 2
