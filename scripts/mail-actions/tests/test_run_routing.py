...
# --------------------------------------------------------------------------- #
# The initiative tagger is RETIRED (2026-09-07) — this pins that it stays gone.
# --------------------------------------------------------------------------- #
# `related_initiative` used to be filled by ranking the mail subject against the
# `initiatives.current` view. The board that wrote that view is gone and so is the
# router module, so a re-added tagger would rank against a FROZEN snapshot and label
# today's mail with last-known initiatives. The two cases below are the ones the old
# tagger got RIGHT, so they are the ones a naive restoration would get wrong: a
# subject that WOULD have matched confidently, and a run whose fake store still
# offers candidates. Both must come out untagged.
...
def test_a_subject_that_WOULD_have_matched_is_left_untagged(monkeypatch, capsys):
    rows = [
    ...
    db = FakeMailDB(rows, initiatives=_WOULD_HAVE_MATCHED)
    _run(monkeypatch, db, CountingLLM(action_required=True))
    assert db.actions[30]["related_initiative"] is None, (
        "an initiative tag was written — the retired router is back, and it is "
        "ranking against a view nothing refreshes any more"
    )
    assert db._mail[30]["labels"] == ["action-required"]


def test_the_run_summary_no_longer_reports_a_routed_counter(monkeypatch, capsys):
    """The counter went with the tagger. A `routed:` line reappearing means the
    tagging path is live again, which the test above would also catch — this one
    catches it at the REPORTING surface, where an operator would read it."""
    rows = [
        {"id": 31, "message_id": "<rc@x>", "from_addr": "sender@example.com",
         "subject": "clawgate chat polish", "received_at": 100,
         "category": "personal", "headers": {}, "text_body": "x"},
    ]
    db = FakeMailDB(rows, initiatives=_WOULD_HAVE_MATCHED)
