...
# 🔴 THE INITIATIVE ROUTER IS GONE (2026-09-07). Actions used to be tagged with the
# initiative they related to, via `scripts/initiatives/route.py` ranked against the
# `initiatives.current` view. BOTH sides of that retired with the initiatives board:
# the router module no longer exists, and nothing refreshes the view, so a surviving
# tagger would have labelled today's mail against a frozen snapshot. `related_initiative`
# is therefore always None for NEW rows; the COLUMN and the digest's rendering of it
# stay, so historical rows still display their tag.
    ...
    with MailDB() as db:
        ...
        for r, _res in survivors:
            ...
            if ex.action_required:
                # Always None since the initiative router retired (see the note at the
                # top of this module). Kept as a named local so the write below and the
                # clawgate card keep one obvious place to re-wire if tagging returns.
                related = None
