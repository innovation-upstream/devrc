...
# --- GUARD 9: NO TEST MAY TOUCH THE OPERATOR'S GIT CONFIG OR A REAL REMOTE -----
...
echo "run-tests: git isolated for this run (GUARD 9)"
...
run_pytest() {
  ...
  # GUARD 8 and `-p testlib.nogit_plugin` is GUARD 9 (see each header). All three
  # are on THIS line — the one place every target is invoked — and not in two
  # dozen conftests, which is what left GUARD 7 covering 1 target of 17 and
  # GUARD 8 covering 1 directory of 13.
  python -m pytest "$d" -q -p no:cacheprovider -p testlib.nolaunch_plugin -p testlib.spool_plugin -p testlib.nogit_plugin \
...
for HOOK_TEST in "${HOOK_TESTS[@]}"; do
  ...
  # GUARD 9 likewise: no plugin, so no session marker and no controls — their
...
# --- GUARD 9 (evaluation): per-target git isolation accounting -----------------
# One line per target, ALWAYS printed — including the zeros, and never the zero
...
for t in "${TARGETS[@]}"; do
  ...
  [ "$seen" -eq 1 ] || nogit_problems+=("$t  — never accounted: run_pytest returned before GUARD 9 could measure it")
done
...
for entry in ${NOGIT_SEEN[@]+"${NOGIT_SEEN[@]}"}; do
  ...
  if [ "$is_pytest" -eq 1 ]; then
    if [ "$markers" -ne 1 ]; then
      nogit_problems+=("$gt  — the nogit plugin emitted $markers session marker(s), expected exactly 1. This target ran WITHOUT the guard, so its real-config-changed=$gchanged means nothing (see GUARD 9's header: -p testlib.nogit_plugin on the pytest line).")
...
if [ "${#nogit_problems[@]}" -gt 0 ]; then
  echo "  ERROR: ${#nogit_problems[@]} GUARD 9 problem(s):" >&2
