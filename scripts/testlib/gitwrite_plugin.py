...
Read `testlib/gitwrite.py` first — it carries the measurement, the reflog excerpt
...
🔴 TWO LAYERS
--------------
  L1 — PROCESS BOUNDARY (`gitwrite.install()` + PATH + `GIT_EXEC_PATH`).
       ...
       git's OWN children — see `gitwrite.exec_path_farm()` for the measurement
...
SESSION_MARKER = "gitwrite(session)"
...
def _log_marker(stub_dir: Path, note: str, via: str) -> None:
    ...
    a runner reads.
    """
    log = gitwrite.log_path(stub_dir)
...
def _inherited_stub_dir() -> Path | None:
    ...
    **rc 0, no banner**, the session marker was written anyway, and pytest
    ...
    It now asks the shim to prove itself (`gitwrite.handshake`), which no foreign
    ...
    """
    raw = os.environ.get(gitwrite.STUB_DIR_ENV)
    if not raw:
        return None
    d = Path(raw)
    answer = gitwrite.handshake(d)
    if answer is None:
        raise ForeignGitShim(
            f"{gitwrite.STUB_DIR_ENV}={raw!r} does not hold THIS policy's git "
            f"shim: it did not answer `{gitwrite.HANDSHAKE_FLAG}` with "
            f"`{gitwrite.HANDSHAKE_MAGIC} {gitwrite.policy_fingerprint()}`. That is "
            "either a foreign `git`, or a shim generated by a different "
            "revision of testlib/gitwrite.py. Adopting it would run this session "
            ...
            f"{gitwrite.STUB_DIR_ENV} to let this session install its own.")
    ...
    if str(gitwrite.repo_root()) not in denied:
        return None
...
def _redirect_argv(argv, stub_dir: Path, tmp_root: Path | None = None):
    ...
    try:
        ...
        if resolved.parent in (Path(stub_dir).resolve(),
                               gitwrite.exec_path_dir(stub_dir).resolve()):
            return None
...
def no_real_git_writes(tmp_path_factory):
    ...
    allowed = gitwrite.default_roots(basetemp)
    denied = gitwrite.denied_roots()
    ...
    if inherited is not None:
        ...
        stub_dir = Path(tmp_path_factory.mktemp("gitwrite"))
        ...
        gitwrite.install(stub_dir, allowed=allowed, denied=denied)
        via = VIA_SELF

    scrubbed = (*gitwrite.GIT_LOCATION_ENV, *gitwrite.GIT_EXEC_ENV,
                gitwrite.MODE_ENV, "GIT_CONFIG_COUNT", "GIT_EXEC_PATH",
                "GIT_ALTERNATE_OBJECT_DIRECTORIES")
    prev = {k: os.environ.get(k) for k in (
        "PATH", gitwrite.STUB_DIR_ENV, gitwrite.ROOTS_ENV, gitwrite.DENY_ROOTS_ENV,
        *scrubbed)}
    ...
    os.environ[gitwrite.STUB_DIR_ENV] = str(stub_dir)
    ...
    farm = gitwrite.exec_path_dir(stub_dir)
    if (farm / "git").exists():
    ...
    os.environ[gitwrite.ROOTS_ENV] = allowed
    os.environ[gitwrite.DENY_ROOTS_ENV] = denied
