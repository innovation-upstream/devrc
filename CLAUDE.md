...
- **Three docs have enforced byte ceilings.** Two are always-on, loading every session: `scripts/browser-bridge/SKILL.md` (`scripts/browser-bridge/tests/test_skill_size.py`) and `claude/RULES.md` (`scripts/tests/test_rules_size.py`). The third is a skill BODY on different economics — `claude/skills/handoff/SKILL.md` (`scripts/tests/test_handoff_skill_size.py`): free until invoked, but `/handoff` runs at the END of a session, when the window is already tight. Each test OWNS its constants and prints an eviction playbook on failure — **read the numbers there, never restate them.** Any addition needs an eviction in the SAME commit.
