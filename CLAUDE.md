...
- **NEVER `sudo nixos-rebuild` from Claude** — can't sudo non-interactively. Stage a system-level change instead: write an executable `nix/system/apply-<change>.sh` that makes the `/etc/nixos` edit and runs the rebuild, hand the user the exact path to run under `sudo`, and say plainly that it is staged-not-applied. `nix/system/` already holds 17 of these — copy their shape, and copy it from **`apply-nebula-drop-443.sh`**, which is the current exemplar: a rootless `--check` mode that reads the RUNNING process (not the unit file), a host guard on a value that actually discriminates (`hostname` does not — several machines here answer `nixos`), an EVALUATION of the *unpatched* config so a later evaluation failure is attributable (it is `dry-build`, which does not build — a compile failure still surfaces at switch time, and the script says so rather than overclaiming), patch-a-temp → parse → unique backup → move → rebuild, and an EXIT trap that rolls back. 🔴 Copy its **errexit discipline** too, and copy it WHOLE: `rc=0; check_one || rc=$?` — never a bare `check_one; rc=$?` (under `set -e` that exits before the assignment and silently makes the whole verification block dead code), and never `check_one || rc=$?` without the `rc=0` first (under `set -u` that is an unbound-variable crash on the SUCCESS path, because `||` does not run when the command succeeds). The half-rule was itself shipped here once and caught by the next audit round: it traded one bug for its mirror image. 🔴 **Do NOT copy `apply-nebula-443.sh`** — it is SUPERSEDED and refuses to run; it is kept only so its reasoning survives beside the artefact. home-manager (user-level) is fine.
...
- **Several docs have enforced byte ceilings, each gated by its own test that OWNS the constants and prints an eviction playbook on failure** — `claude/RULES.md` (the only always-on one) plus the **six** skill bodies `browser`, `prune-skill`, `session-manager`, `handoff`, `clawgate` and `resume` (`scripts/tests/test_resume_skill_size.py`, added 2026-09-17 with the prune that took that body 51,356 → 20,731 B), plus one skill **sidecar** (`scripts/browser-bridge/reference/validation-prompt.md`) and the whole `claudedocs/**/handoff-*.md` corpus — **nine** in all. 🔴 **Read the numbers in the tests, never restate them.** ⚠ `resume`'s ceiling sits well ABOVE the 12,288 B skill target and says why in its own ledger: ten test modules pin literal strings its TOOLS emit into that body, most of them two-way ledgers scraped from the tool, so the floor is set by the pins and not by the prose — trimming it is how an instruction gets lost, and going lower means MOVING pins (the pattern is in `test_subsystem_recall.py`'s `RESUME_SENTENCES_REFERENCE` split), not cutting text. ⚠ **One of them is not a single file: `scripts/tests/test_handoff_doc_size.py` caps EVERY `claudedocs/**/handoff-*.md` at once**, with an enumerated grandfather ledger for the docs that already exceed it — so it is the one whose failure names a document you did not edit. (It used to match NEITHER pattern in the grep below; it now matches `MIN_HEADROOM` only because a docstring in it *names* the pattern, and its actual constants live in `scripts/lib/handoff_budget.py` — so the grep finds it by accident, which is not a discovery method.) 🔴 **And NO SINGLE GREP FINDS THEM ALL — this line said `git grep -l MIN_HEADROOM_BYTES scripts/` "answers it" and that was FALSE, measured 2026-09-07: the ceilings use different mechanisms, so that pattern misses `session-manager` (which this very bullet names — it uses `MIN_HEADROOM_ROWS`) and `clawgate` (asserted as a bare `st_size <=` inside `scripts/claude-hooks/tests/test_clawgate_task_interview_guard.py`, a hook-guard test, not a `*_skill_size.py`).** 🔴 **RE-DERIVED 2026-09-17, and the union grep does NOT find them all either: there are NINE ceilings, EIGHT of which the grep returns.** The ninth is `scripts/browser-bridge/tests/test_validation_prompt.py`, which owns `MAX_DOC_BYTES` and asserts `size <= MAX_DOC_BYTES` over `scripts/browser-bridge/reference/validation-prompt.md` — a skill **sidecar**, matching neither pattern; it was found by sweeping module-level `MAX_*BYTES` constants, a second method with its own misses. Of the eight the grep does return, `MIN_HEADROOM` alone finds seven (missing clawgate) and `st_size <=` alone finds two, missing six. The full derivation, with every over-match named, is in `scripts/tests/test_resume_skill_size.py`'s header:
  ```bash
  git grep -lE 'MIN_HEADROOM|st_size <=' -- scripts/   # ⚠ union, and NOT provably complete
  ```
  ⚠ It over-matches too — it returns 12 paths for 8 gates: `dl-router/server.py`, `present/measure.py`, `skill-audit.py` and `scripts/tests/test_skill_audit.py` are not doc ceilings (the last three READ another gate's constants rather than owning one). So read each hit rather than counting them. This bullet has now been wrong FOUR times: twice carrying a count that was stale within a day, once — for longer, because nobody re-ran it — carrying a discovery method that silently returned an incomplete set, and once (2026-09-17) carrying a count that was two low because the number was copied forward instead of re-derived. A grep quoted as authoritative is a claim like any other; the only fix that would stop this recurring is a test enumerating ceilinged docs two-way, which does not exist yet. Any addition needs an eviction in the SAME commit; raising a ceiling needs the commit message to say which instruction would not fit.
- **Run the gate with `scripts/gate.sh`** (`--tier pytest|node|go|all`, `--set hermetic|all`). 🔴 **THERE ARE THREE TIERS NOW, NOT TWO** — `go` (`scripts/run-go-tests.sh`, `checks.gotests`) joined `pytest` and `node` with the `mention-review` TUI. **`--tier both` is an ALIAS for `all`**, deliberately: a caller who typed `both` before Go existed wanted the whole gate, and silently excluding a language from it is the "list that only grows when somebody remembers" defect. ⚠ **Whether CI builds the `gotests` leg is decided in the infra repo** (`devrc-ci-pipeline.yaml` hardcodes its legs) — `gh pr checks <n>` is what answers that; until you have checked, assume it gates nothing. It sends the full output to a LOG FILE and prints only a bounded summary, so there is no reason to pipe it — and **its exit status is authoritative**. It also cross-checks that status against the runners' own `RESULT:` line and exits **90 = could-not-vouch** when they disagree, when a run printed no verdict, or when `panic: test timed out` appears. 90 is not "the tests failed"; it means read the log. 🔴 It also exits **91 = PARTIAL** when every tier passed but one did not report `SCOPE: FULL` — a narrowed run, or one that printed no scope at all. Neither 90 nor 91 is a gate PASS. It refuses outright (exit 2) if any of `DEVRC_TARGETS`, `DEVRC_GATE_PYTEST_RUNNER`, `DEVRC_GATE_NODE_RUNNER` or `MIN_TESTS` is set in its environment — each changes what the gate runs or accepts while `GATE: RESULT=PASS` still reads as a full verdict, and a replaced runner can print a green one having run nothing. Measured 2026-09-08: `DEVRC_TARGETS=<one target> scripts/gate.sh --tier pytest` printed `GATE: RESULT=PASS exit=0` off 12 tests. ⚠ A zero-test invocation (`--check-targets`/`--check-floors`/`--check-suites`) now reports `SCOPE: NONE`, never `FULL`.
...
- 🔴 **NOTHING BLOCKS A MERGE TODAY — `main` is protected in NAME ONLY, and the local full-suite ritual that used to stand in for it is DELETED. CI is advisory; read it.** <!-- merge-gate: other -->
  ...
  **If you do run a full tier, say which command you ran** — see the DELETED-RITUAL note
  ...
  differently there. **IF you run the gate at all, run BOTH tiers, and name the tier and the
  base sha in the claim** — "the gate passed" is true of one run, one tier, one base, and
  reads as a property of the change. ⚠ **This is about how to make a CLAIM, not a mandate to
  run it.** An earlier wording said "Run BOTH before you claim a merge is safe", which
  contradicted the bullet ~190 lines above deleting the pre-merge full-suite ritual — two
  paragraphs of this file giving opposite instructions, found by an audit rather than by
  anyone reading it. The ritual is deleted; the two-tier rule governs what a gate run you
  DID do is evidence for.
