...
- 🔴 **NOTHING gates a merge in this repo — the gate is YOU, run by hand.** There is no
  `.github/workflows`, no pre-push hook (`.git/hooks` holds only `*.sample`), and no Tekton
  trigger for devrc; `gh pr view <n> --json statusCheckRollup` returns **0 checks** on every
  PR, merged ones included. This line used to read `CI gates both suites`, which was false
  and is exactly the kind of claim nobody re-checks — an agent reads it, believes the merge is
  protected, and skips the run. **Run the gate yourself before you merge, and say which
  command you ran**: `nix build .#checks.x86_64-linux.pytests` and
  `.#checks.x86_64-linux.nodetests` (or `scripts/gate.sh`) are the authoritative commands, and
  they remain worth using for the reason below — they assert collected-test FLOORS and parse
  structured output rather than reading an exit code, because `node --test <dir>` silently
  yields a bogus `# tests 1` and a pytest suite can collect 0 with a zero exit. Gate on the
  MERGED tree, not the PR branch. Pinned by `scripts/tests/test_ci_claim_matches_reality.py`,
  which fails if this paragraph and the repo ever disagree again — in either direction, so
  wiring up real CI is what makes it safe to claim CI.
