...
|---|---|---|---|---|
| `~/.config/activity-collector/env` | `CLICKHOUSE_URL`, `CLICKHOUSE_USER`, `CLICKHOUSE_PASSWORD`, `CLICKHOUSE_DATABASE`, `CLICKHOUSE_TABLE`, `ACTIVITY_HOST`, `ACTIVITY_BATCH_SIZE`, `ACTIVITY_FLUSH_SECONDS`, `ACTIVITY_MAX_BUFFER_BYTES`, `ACTIVITY_MAX_BUFFER_AGE_SECONDS`, `ACTIVITY_HTTP_TIMEOUT`, (`ACTIVITY_SPOOL_DIR`) | both (workbench + laptop) | **auto** (from `scripts/collector/.env.example`, chmod 600) | Only `CLICKHOUSE_PASSWORD` is sensitive: the authed **writer** cred lives in the SOPS secret `homelab-talos/clusters/homelab/apps/activity/secrets.enc.yaml` (decrypt with `SOPS_AGE_KEY_FILE=~/workspace/homelab-talos/.secrets/age.key`). `.env.example` default is empty (unauthed `default` user). `ACTIVITY_HOST` is **derived, not templated, and nothing writes it into this file**: the template states none (a template cannot know which machine it lands on, and both are hostname `nixos`), and `collector.py` asks `scripts/lib/host_label.py` for the label whenever the variable is unset — it names the machine from an address the machine holds. Set it by hand only to override. **Laptop** must point `CLICKHOUSE_URL` at its nebula CH endpoint. See the `activity` skill. |
...
3. **Activity telemetry** (edit the seeded file): leave `ACTIVITY_HOST` unset —
   the collector derives it from an address this machine holds. Confirm with
   `systemctl --user show activity-collector` running and no
   `could not derive this host's ACTIVITY_HOST label` warning in
   `journalctl --user -u activity-collector`; set it by hand only if that warning
   is there. On the laptop
   repoint `CLICKHOUSE_URL` at the nebula CH endpoint. Slot in the authed `CLICKHOUSE_PASSWORD` (writer cred from
   the SOPS secret) to ship to the authed store. `systemctl --user restart activity-collector`.
4. **clawgate** (for approval push + the drafter's card producer): create
   `~/.claude/clawgate.env` with `CLAWGATE_API_URL` + `CLAWGATE_HOOK_TOKEN`
   (via stdin). Needed before the PermissionRequest hook or
...
5. **Kubeconfigs**: clone `homelab-talos` (→ `$KC_HOMELAB`, `$KC_WORKBENCH`) and,
   ...
   Required before: mail-actions (reads k8s secrets), the drafter's digest relay,
   any `KUBECONFIG=$KC_* kubectl` call.
6. **OpenRouter key — 1 of 2, the `repo-cos/env` one** (workbench/homelab):
   create `~/.config/repo-cos/env` with
   `OPENROUTER_API_KEY` (chmod 600) — see its row above. This step was deleted
   by `bac41175` with the retired `repo-cos` timer, which was the only thing
   that ever sourced the file; the key outlived it, so the step is restored.
   🔴 **Creating the file is NOT sufficient — nothing sources it.** Every
   consumer reads the ENVIRONMENT, so you must also export it in the shell that
   runs them: `set -a; . ~/.config/repo-cos/env; set +a`.
7. **OpenRouter key — 2 of 2, opencode's auth store** (any host running
   `opencode`): run **`opencode auth login`**, choose the OpenRouter provider and
   paste a key. That writes `~/.local/share/opencode/auth.json` (mode 600) — see
   its row above. 🔴 **This is a SEPARATE credential from step 6 and the two are
   not interchangeable**: step 6's key never reaches opencode, and this one never
   reaches `scripts/mail-actions/`. Skipping this step leaves every
   `scripts/opencode/opencode-dispatch` alias and `scripts/browser-bridge/browser-agent`
   failing at provider auth — loudly, at first use, which is how you will find
   out. Verify with `opencode auth list`.
8. **mail-actions** (workbench/homelab): no local file of its own — ensure
   `KUBECONFIG` reaches homelab and `OPENROUTER_API_KEY` is **exported** (step 6
   is where the value comes from, and its export line is what puts it in the
   environment). Without the export, Stage 2 exits 2 with
   `ERROR: OPENROUTER_API_KEY not set` even though step 6 was done.
9. **Graphical bar extras** (workbench, optional): `~/.config/bar/media.env` and
   `~/.config/bar/airvpn.env` (0600) if you use the media/AirVPN bar blocks.
10. **git pre-push hooks** (optional): `githooks/install.sh`, then copy
