...
3. **Activity telemetry** (edit the seeded file): `ACTIVITY_HOST` is appended for
   you by the activation, derived from an address this machine holds — check it
   is there (`grep ACTIVITY_HOST ~/.config/activity-collector/env`) and set it by
   hand only if the switch reported it could not derive one. On the laptop
