...
| `~/.config/repo-cos/env` | `OPENROUTER_API_KEY` — measured 2026-09-19, that is the file's ONLY assignment | workbench only | manual (chmod 600) | 🔴 **DO NOT DELETE.** Corrected 2026-09-19: this row used to say the file was dead and safe to `rm` (`git show bac41175 -- SECRETS.md`), and it is not — the key is live. What was retired 2026-09-07 is the weekly repo chief-of-staff and its timer, i.e. the credential's *consumer*, not the credential — and the "nothing reads that file" half was refutable from THIS FILE on the day it was written, because `scripts/mail-actions/` has read `OPENROUTER_API_KEY` throughout (`extract.py`, `llm.py`; see the mail-actions rows below). A newer consumer is `<civitai/cli>/scripts/dogfood/runner.py`. Both read the ENVIRONMENT; nothing sources this file automatically, so the coupling is a human step. Source of truth: the OpenRouter dashboard. 🔴 **Rotation coupling:** this host holds **more than one** OpenRouter key — at least this file and `~/.local/share/opencode/auth.json` (`openrouter.key`), measured 2026-09-19 as DIFFERENT values — so "rotate the OpenRouter key" is ambiguous and rotating this one covers only its own consumers. Rotation of THIS key is an open item recorded with its decision in `claudedocs/close-the-loop/STATE.md`. |
...
6. **OpenRouter key** (workbench): create `~/.config/repo-cos/env` with
   `OPENROUTER_API_KEY` (chmod 600) — see its row above. This step was deleted
   by `bac41175` along with the retired `repo-cos` timer, on the premise that
   nothing read the file; that premise was wrong, so the step is restored.
   Required before: step 7 (mail-actions Stage 2 exits with
   `ERROR: OPENROUTER_API_KEY not set`).
7. **mail-actions** (workbench/homelab): no local file of its own — ensure
   `KUBECONFIG` reaches homelab and `OPENROUTER_API_KEY` is exported for the
   extractor (step 6 is where that value comes from).
