...
| `~/.config/repo-cos/env` | `OPENROUTER_API_KEY`; plus the non-secret `REPO_COS_SEND`, `REPO_COS_FROM`, `REPO_COS_REPLY_TO`, `REPO_COS_PROD_KUBECONFIG`, `REPO_COS_RELAY_NS`, `REPO_COS_RELAY_SVC` | workbench only | manual (chmod 600) | 🔴 **DO NOT DELETE — this row said "NO CREDENTIAL FILE ANY MORE … Safe to `rm ~/.config/repo-cos/env`" and that was WRONG, corrected 2026-09-19 by reading the live key's own `/api/v1/key` endpoint: the file exists, and the key in it is VALID, PAID and in use.** What was retired 2026-09-07 is the weekly repo chief-of-staff and its timer — its *consumer*, not the credential. **A second consumer has since arrived**: the blind dogfood harness at `<civitai/cli>/scripts/dogfood/runner.py` (another repo) reads `OPENROUTER_API_KEY` from the environment, and this is the only file on this host that carries one. Following the old instruction destroys a working credential whose only other copy is the OpenRouter dashboard. **The transferable lesson: retiring a consumer does not retire a credential — check for a new reader before recording a file as dead.** The env NAMES survive because the module that reads them, `scripts/task-spec-drafter/email_send.py` (the DKIM relay send path), moved to the drafter and kept them; renaming them is a separate change. The drafter's unit sets `REPO_COS_PROD_KUBECONFIG` itself, and none of the `REPO_COS_*` values is a secret. Source of truth for the key: the OpenRouter dashboard. ⚠ **Rotation is an OPEN hygiene item the operator has previously DECLINED** (`claudedocs/close-the-loop/STATE.md`): the key is recorded as having appeared in transcripts, and the file has not been written since 2026-07-01. Recording it here because this row is where someone looks — the new consumer above widens what a rotation would have to cover, it does not change the decision. |
...
6. **mail-actions** (workbench/homelab): no local file — ensure `KUBECONFIG`
   reaches homelab and `OPENROUTER_API_KEY` is exported for the extractor.
7. **Graphical bar extras** (workbench, optional): `~/.config/bar/media.env` and
   `~/.config/bar/airvpn.env` (0600) if you use the media/AirVPN bar blocks.
8. **git pre-push hooks** (optional): `githooks/install.sh`, then copy
