...
- **`resolved`** — for each entry listed, propose appending **one dated bullet** in the existing style (`- YYYY-MM-DD: …`, ≤2 lines) as the **FIRST** bullet under `## Nuance / work-history`. Write a *durable lesson* — a gotcha, a decision, why it was touched — never routine status, never a config value, never a copy of what the handoff doc already says.
  ...
  🔴 **CHECK THE MARKER AFTER YOU WRITE IT — nothing validates one at WRITE time.** `cairn append`/`cairn put` enforce only the size cap and `If-Match`; the server parses the entry's EXISTING bullets (for duplicate detection, `append_bullet`) and never validates the INCOMING marker, so one that fails to parse is accepted silently. It fails in the hiding direction: the bullet reads perfectly as prose, so a near-miss looks like a successful close to whoever wrote it. 🔴 **The post-write check is `cairn-validate --scope <scope>` (mandated below) — it is the ONLY surface that both runs after the write AND quotes the offending bullet with a remedy.** Measured 2026-09-16: `cairn recall --list` shows the `🔴 N NEAR-MISS` badge but carries NO remedy text, and **`cairn recall --ref <entry>` shows no badge at all** — so re-reading the single entry you just wrote is a SILENT PASS. The per-entry `OPEN:` block below runs on the resolve pass, i.e. BEFORE the append, not after it. 🔴 **The tool's remedy text is necessary but NOT sufficient, so keep this one rule here:** the token between the marker and the colon must be **7-40 hex characters, or absent** (`_JOURNAL_OPENNESS`). `RESOLVED PR#505:` is upper-case, unemphasised, unparenthesised and colon-terminated — it satisfies every property that message states — and still parses to `(None, None)`. 🔴 **When NO commit implements the closure** (a hand-run database change, an operator console action) **write a bare `RESOLVED:`; do NOT reach for a nearby sha.** `resolved_by` exists so a reader can `git cat-file -e` the closure, and `scripts/subsystem-audit.py` does exactly that — an adjacent-but-unrelated sha passes, recording a VERIFIED closure pointing at a commit that implements nothing, and is marked `EVICTABLE`. A bare `RESOLVED:` reports as `⚠ N UNVERIFIABLE`, which the tool calls **"Not a defect - closing is the point"**. ⚠ Name a durable record in BACKTICKS when you do: with neither a sha nor a backticked path/PR token, `subsystem-audit.py` returns `NO_HOME` — prose in the body does not clear it.

  🔴 **Before appending, act on the `OPEN:` block the tool prints for that entry.** It lists, as SEPARATE populations that never overlap: every declared-open bullet with its age (`oldest unverified for N days`); every bullet that **tried to write a marker and missed the grammar**, which declares nothing — fix the line (⚠ it DOES raise a `🔴 N NEAR-MISS` badge on `cairn recall --list`, measured 2026-09-16; this clause and the tool string it quotes, `subsystem_touch.py`'s "show no badge", are both wrong and the tool is the root); and every bullet that merely *looks* like an open action, which is **AT LEAST this many**, a two-phrasing floor with unknown recall, never a count. Re-check each against the repo *now*: you are the next writer, and the next write is the only moment anyone is looking. Closing one is a one-line edit and is worth more than the bullet you came to add.
...
🔴 **NAME THE IDENTIFIER A FUTURE READER WILL SEARCH — not only the one you were standing in.** A bullet is retrieved by `cairn search`, which is fuzzy text matching over the bullet body, so **a fact nobody can spell is a fact nobody will find**, and the writer's vocabulary is the *work's* vocabulary while the reader's is the *symptom's*. Measured 2026-09-07 on `datapacket-talos`: its alert history was written by sessions **authoring rules**, so it is keyed by metric (`noDataState` → 13 hunks, `node_network_receive_drop_total`, `kubelet_volume_stats_available_bytes`) — and only **9 of 35** alert titles in the `investigate-alert` catalog match anything at all, though those titles are exactly what an on-call has in hand. `Cilium BPF Map Pressure High`, `DatasourceNoData` and `CivitaiDpProdHighLatency` return NOTHING under any spelling. So: write the metric **and** the alertname; the service **and** the symptom; the file **and** the error string it emits. It costs a few words in a bullet you are already writing, and it is the difference between a record and a record that gets read. ⚠ This does not license padding — name identifiers the work actually involved, never keywords you hope someone types.
