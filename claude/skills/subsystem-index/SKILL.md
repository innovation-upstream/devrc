...
- **`resolved`** — for each entry listed, propose appending **one dated bullet** in the existing style (`- YYYY-MM-DD: …`, ≤2 lines) as the **FIRST** bullet under `## Nuance / work-history`. Write a *durable lesson* — a gotcha, a decision, why it was touched — never routine status, never a config value, never a copy of what the handoff doc already says.
  ...
  🔴 **BUT THAT BLOCK IS COMPUTED FROM THE FROZEN MIRROR, NOT THE LIVE STORE — so it can name bullets that are ALREADY `RESOLVED`, and following it literally re-opens settled work.** `subsystem_touch.py` reads `~/.claude/analyze-service-index` and prints that path on its own `store:` line; `cairn` reads `~/.cache/subsystem-store`. Since the Cairn cutover the first is a frozen read-only mirror, so **every population in the `OPEN:` block, the `already there` display and the `journal: N bullets, newest dated …` line are as stale as that mirror.** MEASURED 2026-09-09: a probe reported an entry had **2** bullets still declaring `OPEN:`, oldest unverified for 23 days — and the live store already carried that exact bullet rewritten as `RESOLVED <ref> (<date>)`, closed five days earlier by another session. The two copies of that one entry were 162 vs 170 lines, and the mirror's newest file was a week behind the cache's. Nothing in the output says the reading is second-hand; the `store:` line is the only tell, and it looks like provenance rather than a warning.
  **So before acting on ANY `OPEN:` row: `cairn sync`, then read `~/.cache/subsystem-store/<scope>/<entry>.md` and check the bullet there.** A row the live copy shows as `RESOLVED` is already done — say so and move on. This costs one command and is the difference between closing a stale marker and re-minting work somebody finished. ⚠ The same staleness inflates the *`already there`* comparison in the opposite direction: a bullet appended since the last mirror refresh is invisible to it, so the duplicate-check it exists for can pass on content that IS already recorded — another reason to read the cache copy rather than the block.
