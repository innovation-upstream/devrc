...
- **`resolved`** — for each entry listed, propose appending **one dated bullet** in the existing style (`- YYYY-MM-DD: …`, ≤2 lines) as the **FIRST** bullet under `## Nuance / work-history`. Write a *durable lesson* — a gotcha, a decision, why it was touched — never routine status, never a config value, never a copy of what the handoff doc already says.
  ...
  🔴 **RE-READ THE ENTRY'S INDEX ROW AFTER ANY MARKER WRITE — nothing checks a marker at WRITE time.** `cairn append` and `cairn put` compute no openness at all, so a marker that fails to parse is accepted silently and you learn nothing until a later read. It fails in the direction that hides it: the bullet reads perfectly as prose, so a near-miss looks like a successful close to whoever wrote it, and the ONLY tell is the row badge. **Do not restate the grammar here — the tool already prints the authoritative remedy**, on the per-entry `OPEN:` block, `--validate` and `cairn recall` alike, quoting the offending bullet beneath it; the next bullet already tells you to act on that block. ⚠ It is WIDER than the shape people usually hit (a parenthetical between the sha and the colon), so read it rather than a remembered rule. 🔴 **When NO commit implements the closure** — a hand-run database change, an operator console action — **write a bare `RESOLVED:` and explain in the body. Do NOT reach for a nearby sha to fill the slot.** `resolved_by` exists so a reader can `git cat-file -e` the closure; an adjacent-but-unrelated sha passes that check and records a VERIFIED closure pointing at a commit that implements nothing. A sha-less `RESOLVED:` is reported as `⚠ N UNVERIFIABLE` and the tool calls that **"Not a defect — closing is the point"** — an honest unverifiable beats a false verified.
