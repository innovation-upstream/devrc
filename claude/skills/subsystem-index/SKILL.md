...
- **`resolved`** — for each entry listed, propose appending **one dated bullet** in the existing style (`- YYYY-MM-DD: …`, ≤2 lines) as the **FIRST** bullet under `## Nuance / work-history`. Write a *durable lesson* — a gotcha, a decision, why it was touched — never routine status, never a config value, never a copy of what the handoff doc already says.
  ...
  🔴 **BUT THE `already there`, `OPEN:` AND `NEAR-MISS` DISPLAYS READ THE FROZEN LOCAL MIRROR, NOT THE POD — so they can be many bullets stale, and acting on them means re-fixing what is already fixed.** The probe names its source on its own `store:` line (`~/.claude/analyze-service-index`), and since the Cairn cutover that tree does not move. MEASURED 2026-09-12 on one scope: the mirror held **11** bullets, newest dated 2026-08-31, against **29** on the pod, newest 2026-09-11 — 12 days and 18 bullets of drift, and the mirror's scope listed 11 entry files where the pod had 36. Both items that run's probe flagged were **already fixed upstream**: a `NEAR-MISS` marker that parses live, and an `OPEN:` bullet the pod already carried as `RESOLVED <sha>:` *plus* a later bullet closing its open half. A writer who acted on that display would have rewritten correct prose and re-opened a closed action — the exact failure the `already there` comparison exists to prevent, arriving through its own input. 🔴 **`cairn sync` FIRST, then diff `~/.cache/subsystem-store/<scope>/<ref>.md` against what the probe printed, before believing any of the three.** The write half already tells you to sync before a `put`; this is the same read, one step earlier — and it is also what makes the `already there` comparison meaningful rather than a comparison against a snapshot of the past.
