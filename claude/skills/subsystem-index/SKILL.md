...
- **`resolved`** — for each entry listed, propose appending **one dated bullet** in the existing style (`- YYYY-MM-DD: …`, ≤2 lines) as the **FIRST** bullet under `## Nuance / work-history`. Write a *durable lesson* — a gotcha, a decision, why it was touched — never routine status, never a config value, never a copy of what the handoff doc already says.
  ...
  🔴 **THE COLON MUST FOLLOW THE SHA IMMEDIATELY — `RESOLVED <sha> (<ref>):` IS A NEAR-MISS AND DECLARES NOTHING.** Measured 2026-09-16 while closing two bullets on one entry: a parenthetical between the sha and the colon, and a DATE written where a sha belongs, took that entry's index row from `🔴 1 OPEN` to **`🔴 2 NEAR-MISS`** — the bullets closed nothing AND declared nothing. Rewriting them as `RESOLVED <sha>: (<ref>) …`, parenthetical moved AFTER the colon, cleared the badge in a single `cairn put`. **Cite a sha, and put any reference after the colon. If no commit implements the fix** (a hand-run database change, an operator console action) **cite the nearest related sha and explain in the BODY — never substitute a date for the sha.** ⚠ It fails in the direction that hides it: the bullet reads perfectly as prose, so a near-miss looks like a successful close to its writer, and the only tell is the row badge — which you see solely if you re-read the index AFTER writing. **Re-read the row after any marker write.** Not rare: in the one scope checked, six further entries carried the same defect, so their recorded resolutions are invisible to every reader.
