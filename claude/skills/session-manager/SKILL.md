...
description: "Live cross-host view of every tmux window — which have Claude Code running, which ones are WAITING ON A HUMAN, how stale it is, plus the clawgate approval queue, recent agent sessions, and UNSENT PROMPTS. Use for: is anything waiting on me, active sessions, what's running where, tmux state across both hosts, cross-host session status, tail a tmux window, which windows are stale/idle/busy/blocked, did I leave anything half-typed / unsent."
---
...
it**: `pull_requests` → `initiative-scan`, `mail_queue` → `mailbox`, `cluster_alerts` →
`obs-read`, `gui_windows_outside_tmux` → `i3`.
