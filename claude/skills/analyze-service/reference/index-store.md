...
- **Never bare `git reset --hard`, `git clean`, or `git checkout --`** — each destroys **uncommitted** curated content, which is exactly the part no commit and no bundle holds.
- 🔴 **`git reset --hard <ref>` is WORSE than the bare form, not milder — it orphans COMMITTED content.** `backup.py` bundles with `git bundle create --all`, which walks **reachable refs only**, so once the branch has moved back, the orphaned commits are in no future bundle either. After the next daily run they survive in the reflog and nowhere else. Do not read "it is committed, so a bundle has it" as safety against a history rewrite.
