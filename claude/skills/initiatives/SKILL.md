...
| Recap model | **`vllm-recap`** — homelab vLLM, **ns `vllm-recap`, svc `vllm-recap:8000`, served model `recap` = Qwen2.5-7B-Instruct-AWQ**. Wired via the unit env (`INITIATIVES_RECAP_ENABLED=1`, `RECAP_NAMESPACE=vllm-recap`, `RECAP_SERVICE=svc/vllm-recap`, `RECAP_SERVICE_PORT=8000`, `RECAP_MODEL=recap`) — recap.py's in-code defaults are PLACEHOLDERs |
