...
4. **Surface what past sessions already recorded — TWO recall surfaces, BOTH UNCONDITIONAL. Run both now, before the report:**
   ...
   🔴 **THE SECOND COMMAND IS HERE BECAUSE IT DID NOT FIRE WHERE IT USED TO BE, AND THAT WAS MEASURED — do not move it back into step 3's prose.** It lived as a conditional inside step 3 (*"before working any open item…"*) from `#1295` until 2026-09-06, and over its first 34 h **1 of 14 `/resume` runs across both hosts ever invoked it** — the one that did was reading `origin/main` because the `SKILL` block told it its copy was stale, not because the step fired. The trigger was not the problem: all six analysed workbench non-firing runs ran `claim-work` and five made edits, and five of six resumed a doc with an `## Open investigations` section. 🔴 **The discriminator is placement, not the tool:** step 3's *sibling* check (`git log --since`, same trigger, same block, an ordinary command) fired **0 of 6**, while this step — numbered, unconditional, fenced — fired **5 of 6**. **A step that is not firing needs MOVING, not louder marking; emphasis was already at maximum on both step-3 checks.** ⚠ **The denominators, stated so they reconcile:** the workbench half was **8** runs = **1** that fired + **6** analysed as non-firing + **1** that was the measuring session itself, excluded as the instrument. All counts are as of 2026-09-06T04:00Z and the corpus grows, so a later re-run reports larger numbers rather than contradicting these. ⚠ **Residual, not yet measured:** this puts a SECOND command inside a step whose 5/6 was measured with ONE. Re-measure with check 1 of the "How to verify" section in `claudedocs/handoff-handoff-search-index.md` — **not** the handoff you read in step 3 — rather than assuming it inherited the rate.

   **Query the TOPIC, not an open item — that is what makes it unconditional.** Every resume has a topic; not every resume has an open item, and a step that can only run in the second case is the conditional this move exists to remove. **When a specific open item is not covered by the topic query, run the command again here with that item's own words** — that is an extra run of a step you have already reached, not a condition on reaching it.

   **What it is FOR:** the `Ruled out:` bullets scattered across the handoff corpus are what stop you re-running a probe someone already ran, which is why the index is section-grained.

   🔴 **`--exclude-slug` IS NOT OPTIONAL POLISH — WITHOUT IT THE TOP HIT IS THE DOC YOU JUST READ.** Measured over the **20** post-fix `/resume` runs that queried the corpus (2026-09-08): **23 of 60 hit slots** were the session's own handoff, and it was the **#1 hit in 13 of 20** — two queries spent all three slots on it. That is arithmetic, not bad luck: you are told to query the handoff's TOPIC, and the best text match for a doc's topic is that doc. Pass the value step 2 printed on its `handoff:` line — a basename like `handoff-<topic>.md`; the flag also takes a bare slug or a `claudedocs/…` path and derives the slug the indexer used. The freed slots go to documents you have **not** read.

   How to tell it landed: the run prints `excluded=<slug>` on its scope line and an `in_scope_docs` **lower than** `indexed_docs`. ⚠ **Do not expect it to drop by exactly one** — it excludes that slug in **every** repo (a bare slug cannot say which), so two repos sharing a doc basename lose both. Measured on this host today: **0** slugs are shared between the reachable repos, so one is the usual case and not the guarantee. 🔴 **`excluded=` proves the flag PARSED, not that it MATCHED** — the count is the half that proves a match, so read the pair.
