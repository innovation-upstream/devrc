...
4. **Surface what past sessions already recorded — UNCONDITIONAL. Run it now, before the report:**

   ```bash
   cairn recall --repo "<path>"
   ```

   🔴 **A SECOND SURFACE (`handoff_search.py`) WAS HERE UNTIL 2026-09-11 AND WAS RETIRED ON A MEASUREMENT — do not re-add it without redoing that measurement.** It was moved into this step by `#1332` precisely because it did not fire in step 3's prose, and the move worked: adoption went 2/11 → 20/22, and after `#1399` added `--exclude-slug` **16 of 16** resuming sessions ran it with the flag. What never followed was YIELD. Measured 2026-09-11 over the 16 post-`#1399` sessions: the self-hit defect was fully fixed (**0 of 48** hit slots were the session's own doc, against 23 of 60 before; own doc was the #1 hit **0 of 16** times, against 13 of 20) — and **0 of 16** sessions opened, mentioned or acted on a hit. Every one went straight to `claim-work` / `gh pr view`. 🔴 **The cause is retrieval recall, NOT an empty corpus, and that distinction is the whole reason this is a retirement rather than a tuning task:** 44 of those 48 hits were `[gotcha#0]` — the same first section of a Gotchas block — with 29 distinct docs filling 48 slots and two docs appearing 5× each across unrelated repos. In the one case where both surfaces ran on the same question, a session asked about a store-api fsync stall, `cairn search 'fsync'` handed it the right doc, and `handoff_search` ranked that same doc **11th** — unreachable at `--limit 3`. **`cairn recall` above is the surviving surface, and it is the one that delivered.** The tool still exists for ad-hoc use (`scripts/lib/handoff_search.py`); what was retired is paying ~4 KB of every resume for it. Full evidence: `claudedocs/handoff-handoff-search-index.md`.
...
5. **Report**:
   ...
   - Anything **step 4** recalled that bears on the next steps, kept separate from what step 2 measured. 🔴 **Carry its label — `cairn recall` prints `from index`** (curated subsystem pointers), and that provenance is what makes a recalled line checkable rather than something you appear to know. ⚠ If you ran `handoff_search.py` ad-hoc (it is no longer part of this step — see step 4), label that separately as `from handoff docs`: it is a different corpus, section-grained, and possibly from another repo.
