...
| Instant block scripts (read cache, never network) | `scripts/i3status-{clawgate,mail,alerts,civitai,notifs,media,airvpn,telemetry,runaways}` |
...
| Relayed snapshot of the OTHER host | `~/.cache/bar-remote/<host>.json` (0600), written by `bar-remote-pull` |
| Remote-host relay (gather + pull) | `scripts/bar-remote-snapshot` — `--gather` on the observed host, `--pull` on the observing one |
...
| airvpn (host) | `i3status-airvpn` | 10 | n/a | **HOST** AirVPN WireGuard pill (net_vpn), **state-driven**, default-OFF — on BOTH hosts, each describing THAT machine's tunnel. Workbench writer = the poller's `airvpn` source; laptop writer = `airvpn-status-poll` (60s timer, laptop-only). The laptop's killswitch is armed in **`roaming` mode** (derived LAN). **left → `airvpn-menu`**; **right → `airvpn-detail` float**. 🔴 Full detail + the killswitch re-test protocol: **`~/.claude/skills/bar/reference/airvpn.md`** |
| fans (workbench) | `i3status-fans` | — | n/a | AIO pump + case fan RPM off the Nuvoton NCT6687D, read straight from `/sys/class/hwmon`. **No signal, no poller, no cache**, so no `?`-on-stale path. `2660·1736` Idle · `!0·1736` **Critical** (pump stalled) · `2660·?` Warning (one tacho unreadable) · `?` Warning (**chip absent — `nct6683` is not loaded**; persist it with `sudo bash nix/system/apply-nct6683-module.sh`). 🔴 **NOT hide-at-zero**, unlike every count pill: a pump RPM is the number itself, and a cooler pill invisible while healthy is invisible in exactly the state it certifies (pinned by `test_this_pill_is_ALWAYS_VISIBLE`). 🔴 **The RPM floor is PER-FAN and OPTIONAL — only the pump has one (500)**; `fan2`/`fan4`–`fan10` read a permanent 0 (nothing plugged in) and a case fan on a zero-RPM curve legitimately stops, so a blanket floor would cry wolf. A fan passed without `:MINRPM` is display-only and can never alarm; a floor of `0` is **refused** (unreachable — `read_rpm` discards negatives). The chip is found by hwmon **`name`**, never by number (it sat on `hwmon11`). `icon = refresh` — material-nf has no `fan` key. 🔴 **Left-click → `fans-detail`, the cooling float** (pump/case RPM + PWM duty, then CPU/GPU/VRM/NVMe temps) — NOT btop, which is a CPU/memory view and shows no fan data at all. `fans-detail` loads `i3status-fans` as a REQUIRED CO-LOCATED SIBLING to reuse the chip-location predicate, so both carry the same `!isLaptop` gate (pinned by `test_fans_detail.py`) |
...
| runaways (workbench) | `i3status-runaways` | 19 | n/a | Runaway-process count, **render-only**: the pill does not decide anything — `scripts/syshealth --json` does, and the poller's `parse_runaways` is pure. Its predicate is `pcpu >= 80` **lifetime-average** AND `etimes >= 600`, over *every* eligible process. 🔴 **It deliberately has NO TOAST, and that is an invariant, not an omission** — `scripts/cpu-monitor.sh` owns the interrupt for this event (6-sample sustain, a per-day cap that demoted alerts on 8 of the last 30 days, and an `IGNORE` list tuned against 12 real false alerts on games). A second, uncapped announcer for the same process regressed that, so the spec was removed and its absence is pinned by `test_the_runaways_source_has_NO_TOAST_SPEC_because_cpu_monitor_owns_it` — **that guard ALONE**. 🔴 The `SOURCES` ledger guard does NOT cover this and must not be read as redundant cover: it asserts `spec.get("source", spec_name) in names`, which a re-added `runaways` spec SATISFIES, since runaways genuinely is a polled source. Measured: re-adding a spec fails the NO_TOAST guard alone, 1 failed / 569 passed. **If a class needs interrupting, WIDEN `cpu-monitor.sh` — do not re-add a spec here.** ⚠ The two announcers are NOT interchangeable: this pill is `!isLaptop`, `cpu-monitor` runs on BOTH hosts, and cpu-monitor reads only `top`'s FIRST data row — so the 80-95% band, the runner-up, rank churn, and any runaway hiding under an ignored top process have no interrupt at all. **left-click → `syshealth` in a held terminal** (pinned BOTH ways: `…CLICK_still_holds_its_terminal_open` pins the command string, `…CLICKS_are_wired_to_syshealthCmd` pins the button wiring — a mutant that rewired the click survived the whole suite until the second one existed) |
| remote host (LAPTOP) | `i3status-remote-host` | — | n/a | One pill relaying the WORKBENCH's HOST-LOCAL state from `~/.cache/bar-remote/workbench.json`, pulled every 60s by the `bar-remote-pull` timer (laptop only). 🔴 **NOT hide-at-zero** — it describes a machine nobody is looking at, so `quiet` and `not heard from in an hour` would both render as nothing; `wb ok` when current and quiet, trailing `?` when not, carrying the last known alarms. 🔴 **It relays VERDICTS, never recomputes them** — the gather runs the remote bar's own block commands read out of its deployed `config-top.toml`, so no threshold lives in this pill. 🔴 **Only HOST-LOCAL blocks travel** (`RELAY_BLOCKS` in `bar-remote-snapshot`): the six GLOBAL-SERVICE pills (clawgate/alerts/civitai/mail/telemetry/media) are deployed on the laptop instead, rendering from a poller cache the same pull syncs — relaying them labelled a shared fact as one host's and made this pill permanently red. `i3status-notifs` is withheld (unseen-dunst backlog, unactionable remotely) and so is this pill itself (recursive). The three classes are pinned two-way against `graphical.nix` by `test_every_custom_block_is_classified_exactly_once`. ⚠ `gpuBlock` is an i3status-rust BUILT-IN with no `command`, so the workbench's GPU is NOT relayed. 🔴 **The rollup surfaces a block only while it is ALARMING**, and `i3status-airvpn`'s steady states are icon-only — so the workbench tunnel's STEADY state is invisible here by design; the laptop stopped mirroring it the moment it got its OWN airvpn tunnel (see the airvpn row — a mirror of the wrong machine's tunnel is worse than none). The gather carries `icon` through; the rollup still names an icon-only alarming block (`test_an_ALARMING_BLOCK_WITH_NO_TEXT_is_NAMED_not_counted`). `runaways` reaches the laptop only through this rollup (alarms-only is lossless for it — no icon-only steady state). **left/right-click → `remote-host-detail`, a float showing both hosts.** Debug: `systemctl --user status bar-remote-pull.timer`, then `bar-remote-snapshot --gather` on the workbench. |
...
not resolve from a nebula-only host. `runaways` remains workbench-only and reaches the laptop
through the `wb` rollup only (alarms-only is lossless — no icon-only steady state). `airvpn`
renders on BOTH hosts as each machine's OWN tunnel: the laptop runs its own tunnel + status
writer (see the airvpn row). ⚠ **Off nebula the laptop's six go `?`/Warning**, since
...
catches a script that was never `git add`ed — but only in the **nix tier**, where a flake omits the
...
## Other gotchas
- 🔴 **THE CLAWGATE PILL POLLS A DIFFERENT SERVICE THAN THE NAME SUGGESTS — and read the WRONG one for days without any visible symptom.** The task board was extracted out of the permission router into a separate service on its own base URL. `scripts/lib/clawgate_tasks.py` now resolves **two** bases from `~/.claude/clawgate.env`: `read_clawgate_env()` → the ROUTER (`CLAWGATE_API_URL`, for `/api/attention`, `/api/tmux/*`, `/api/transcripts/*`, `/api/send`) and `read_clawgate_task_env()` → the TASK service (`CLAWGATE_TASK_API_URL`, falling back to the router when unset, for `/api/tasks*`). **The pill is a TASK-side consumer**, so it must use the task base (`#1877`). Before that fix it appended `/api/tasks` to the ROUTER base and polled the retired board every 45s. 🔴 **Nothing looked wrong**: the retired service still answers `/api/tasks` with 200, and both boards still reported identical counts — so the pill showed a plausible number from the wrong source, and would have started lying silently the moment they diverged. **When adding any pill that reads tasks/agents, take the base from `read_clawgate_task_env()`, never from the router reader**, and remember the fallback means a host that sets only `CLAWGATE_API_URL` gets the old behaviour with no error.
- ⚠ **A "runaway process" toast can be a BACKLOG, not an incident.** Dunst text of the form `at 101.8% CPU for ~180s (running 09:05) … (139h 58m 40s old)` has TWO durations: `~180s` is how long the PROCESS ran, the trailing `139h` is how old the NOTIFICATION is. A machine nobody has touched for days accumulates these (plus a `(65 more)` line) and the screen reads like an active incident. Check live before reacting: `ps -eo pid,etimes,pcpu,args --sort=-etimes | grep <pat>` and count what is ACTUALLY over your threshold.
- **Icon overrides need the TABLE form.** `settings.icons = { icons = "material-nf"; overrides.gpu = "…"; }`.
  ...
  `gpu`→a monitor glyph).
- 🔴 **A pill stuck on `?` on the LAPTOP is usually the PULL, not the poller.** The laptop runs no poller; its global-service pills and its `wb` pill both read caches written by `bar-remote-pull`. Check that timer first — `journalctl --user -u bar-remote-pull -n 20` — and note the unit exits **0** on an unreachable peer by design (a closed lid must not toast), so a permanently-broken pull is visible ONLY in that stderr line and in the `?` itself.
- **The laptop's six global-service pills have NO click handlers**, deliberately: their targets are `grafana.homelab.lan`, `qbittorrent.workbench.lan` and a LAN IP, none of which resolve from a nebula-only host, and two need per-host 0600 credential files that are not there. Dead buttons were the alternative.
