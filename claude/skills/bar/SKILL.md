...
not resolve from a nebula-only host. `airvpn` + `runaways` DO remain workbench-only. `airvpn` ALSO
gets the dedicated laptop mirror `remoteAirvpnBlock` (`--block i3status-airvpn`, every steady
state, icon included); `runaways` reaches the laptop through the `wb` rollup only — it has no
icon-only steady state, so alarms-only there is lossless. ⚠ **Off nebula the laptop's six go `?`/Warning**, since
