...
| load average | `i3status-load` | — | — | 1-min load from `/proc/loadavg`. **No signal, no poller, no cache** — nothing to go stale, so it has no `?`-on-stale path (only on a bad threshold). **BOTH hosts.** Hidden below `loadWarnAbove`; Warning at it; Critical at `loadCritAbove`. 🔴 Both are single-sourced in `graphical.nix` and **pinned to `CPU_MON_THRESHOLD` in `home.nix` by `test_bar_status.py::test_the_load_pill_threshold_MATCHES_cpu_monitors`** — the pill and cpu-monitor's toast are two renderings of ONE decision, and keying the pill off `nproc` instead would warn at 24 on a workbench that idles in the twenties, reinstating the noise the 08-05 threshold bump removed. Passed as `--warning/--critical`, not env. Left-click → btop |
...
catches a script that was never `git add`ed, which a flake omits silently). (b) A block's `icon`
