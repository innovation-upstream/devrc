...
- **Blocks render local files only.** The bar NEVER queries a remote per tick. Anything remote
  ...
  the block renders **empty/invisible** — **except `clawgate` and `telemetry`**, whose
  reassuring answer is a ZERO, so "cannot tell" must not borrow the pixels that mean "nothing
  to tell". Both render a visible `?` instead (see the block table).
