...
2. **Draft the handoff doc into a SCRATCH FILE.** 🔴 **`claudedocs/handoff-<topic>.md` is written by step 5 and by nothing else — whether or not it already exists.** Draft `## ` headings into a scratch file **under your scratchpad directory, never inside the repo** — an in-repo scratch file lands in step 4's session AND git windows, and `--exclude` names the handoff doc, not it — then land it in **step 5**, which owns the merge, the gate and the commit. When the doc EXISTS your scratch file is a *delta* — omit a section and it is left alone. When it does NOT, the delta simply becomes the doc verbatim, so write the whole structure below into it. Use this structure — be concrete, link exact file paths and commands, no vague prose:
   ...
   🔴 **The field may sit ANYWHERE on the item, a wrapped continuation line included** — most real items wrap, so the tag on the last line of a multi-line item counts exactly as much as one on the numbered line. What is checked is the item's own block: it ends at the next numbered item, or at the first unindented line after a blank one, so **a trailing paragraph under the list — including this boilerplate, if you paste it — belongs to no item and tags nothing.** Emphasis around the field is fine (`**forcing: gate**`, `**forcing:** gate`, `` `forcing: gate` ``); a word between the key and the colon is not (`forcing function: gate`), nor is any other separator (`forcing = gate`), nor a field inside a code fence. **Those three are near-misses and the refusal says so by name** — it never tells you to add a field you already wrote.
...
5. **Land the handoff doc — the write+push gate.** MEASURED: a session re-entered from a handoff, did ten minutes of real analysis, then wrote and **pushed** an updated handoff to a shared branch that nobody approved. `/resume` is read-only and followed its contract; nothing gated the doc's own write+push. (This sentence used to add *"step 4's index write is gated"* as the contrast — that gate was retired 2026-08-15 and step 4 is now a pointer to `subsystem-index`, so the contrast had become false in two ways.)
   ...
   **The doc's YAML front matter survives this merge** — `split_front_matter` carries the base's block through, so a delta that starts with prose rather than a `## ` heading can no longer silently drop the `clawgate-task:` field. Put a front-matter block in your delta ONLY when you mean to change the recorded task; an explicit one wins. 🔴 **The NEW-doc case inverts that:** there is no base block to carry, so the delta's own front matter is the doc's only chance at one — if step 1 resolved a task, it must be at line 1 of the scratch file.
   ...
   🔴 **`THE BASE DOCUMENT IS NOT THE NEWEST COMMITTED COPY` / `THIS MERGE LOOKS LIKE IT RESOLVED THE WRONG BASE` — read it before you answer y.** The base comes from `--repo`'s working tree, so a stale clone silently merges into an out-of-date document and reports success. Measured on a clone 313 commits behind: confirming would have discarded ~601 lines and a whole incident writeup, at `status=written`, exit 0. 📖 write-gate §B. So the run names it: the mainline's commit count for **this doc** (mainline **derived**, never assumed `main`), both copies' section/line counts, and which tell fired. **A WARNING, never a refusal and no exit code changes** — a deliberately-behind clone is legitimate. 🔴 **It is a FLOOR: silence is not evidence the base is current**, and it never fetches, so the counts are a lower bound.
   ...
   🔴 **Three refusals enforce the 2026-08-28 cap. All write NOTHING, each prints its own fix, and re-running after fixing your scratch file is safe.** `status=dated-topic` (7) — dated slug ⇒ per-session doc; **no bypass exists**, re-run without the date. `status=new-doc` (7) — no doc for this topic and the repo has others, which it lists; if one IS this effort re-run with ITS topic — 🔴 `--new-effort` asserts genuine newness, it is **not a way past the list**. `status=unforced` (8) — a ranked item names no forcing function or an unrecognised kind. 🔴 **Read the marker on each row, because the fix differs and only one of them is "add a field"**: `[no forcing: field]` ⇒ add one (a continuation line counts); `[unknown kind: …]` ⇒ pick from the vocabulary; `[unparsed forcing field on: …]` ⇒ the field IS there and is misspelled, so re-spell the quoted line as `forcing: <kind>`; `[fenced]` ⇒ move it out of the code fence. ⚠ `forcing: none` items print an **advisory** above the diff; the write proceeds.
   ...
   🔴 **`--confirm` WITHOUT `--push` leaves a real commit in this checkout only — and it now says so.** `status=written commit=<sha> branch=<b>` is followed by `NOT PUSHED`, plus the exact command: a `git push` on a feature branch, or, on a shared branch, the preserve-on-a-topic-branch route (several repos forbid committing to theirs, so it will not hand you a push to `main`). A **SUCCESS, not a refusal** — exit 0 — but push it or open a PR **in this session**: an un-pushed handoff is one only you can read. 🔴 **Do NOT retry by re-running with `--push`** — the doc already carries the update, so a second run exits 5 `no-change` or **appends your findings twice** and pushes the duplicate.

   🔴 **Land it — no question. SHOW the diff, then push.** Operator decision 2026-08-23: always answered `y`, exactly as the index write's was — a round trip and no safety. Re-run the identical command with **`--confirm --push`**: one commit, path-limited, carrying exactly the diff shown. **The two-run shape STAYS**: the proposal run is what puts the diff in the transcript, the only record of what landed.
