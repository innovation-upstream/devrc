...
4. **Record what this session touched in the subsystem index** — a read-only probe, then an opt-in write. The index is the terse *pointer sheet* that outlives this handoff doc, and this step is its second writer.
   ...
   **— the write half; everything above this line only reads —**
   ...
   📖 **The measured evidence behind every rule below is in `reference/index-write.md`** — the two-session concurrency simulation and the retraction it forced, the autocommit timer's behaviour, each source's blind spots with their numbers, and the `gh` file-list truncation cap. Read it when a rule looks arbitrary or you are about to work around one; the rules themselves are complete here.
