...
🔴 **Three of the four blind auditors declined to run the sandbox tier**, and the brief is why: it
told them to run `nix build …#checks…` and "read each runner's own `RESULT:` line", with **no
`-L`** — which prints nothing for a build that succeeds. Fixed in `scripts/audit-dispatch.py`;
see that file's TOOLCHAIN block for the measurement.
