...
| Hook events | `PermissionRequest` (`CLAWGATE_REMOTE_APPROVAL=off`) + `Stop` (async, `CLAWGATE_SUGGEST=off`), both in `~/.claude/settings.json`, ON by default. 🔴 The `Stop` array also carries unrelated hooks — **preserve EVERY non-clawgate entry**, and re-derive the list (`jq -r '.hooks.Stop[].hooks[].command' ~/.claude/settings.json`) rather than trusting a count here: `settings.json` is per-host and unmanaged, so any number written here is wrong on some host. This line said "two" until 2026-08-29, when the live array carried **four** (`task-hook.sh`, `claude-notify.py`, `next-step-nudge.py`, `agent-ledger-hook.py`) — following it literally drops the two it does not name |
