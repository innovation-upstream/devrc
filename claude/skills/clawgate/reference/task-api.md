...
| **edit** | `PATCH /api/tasks/{id}` | 🔴 the body field on the WIRE is **`body`**, not `content` — measured live on **0.7.99**: `{"content":…}` returns **200**, bumps `updatedAt` and CHANGES NOTHING, while `{"body":…}` applies. An unknown key is silently ignored, so the 200 is a claim about the REQUEST, never about the task — the route returns the full updated task, so read `.body|length` back rather than the status code. (This cell said "content" until 2026-08-25.) Also: dispatch config + `tags` (replace) / `addTags` / `removeTags` (merge); **status/provenance/created_at immutable**; `tags`+merge together → 400 (0.7.73/0.7.75). ⚠ **The `in_progress` 409 is REFINED, not blanket**: a **descriptive-tag-only** edit SUCCEEDS while in progress (a label is not a spec change). 409 fires only when a non-tag field is present, or any touched tag is a ROUTING tag — and a `tags` **replace** counts the CURRENT set as touched, so it 409s on any task that already has one |
...
| **merge** | `POST /tasks/merge` | 🔴 **SESSION route, no `/api` counterpart, FORM-encoded** (`winner=`,`loser=`). Merge by SUPERSEDE — nothing is deleted; winner gains the tag union + a comment, loser gets a comment and `complete`. 400 self-merge/bad id · 404 unknown · 409 if EITHER task is `in_progress` or already `complete`. 200 with an EMPTY body + `HX-Trigger`. The machine gap is deliberate: the audit comments are authored `user`, which `taskCommentAuthor` structurally cannot mint (0.7.84). 🔴 **MEASURED 2026-08-26: those two constraints together make merge UNREACHABLE for the normal supersede.** Superseding #359 with #360: session route → **409** (the successor was `in_progress` *because* it superseded, and the loser ended `complete` — both arms fired), `POST /api/tasks/merge` → **405**, `clawgatectl` → no verb. So it was hand-rolled as a comment + status flip, which produced a **WORSE record than the feature**: a `claude-code` comment instead of the `user`-authored audit comment. ⚠ And there is **no status meaning "superseded"** — #359 is `complete`, which is false, and true only because a human wrote prose. Decision card: **#366**. Do not file "build supersede" — it exists |
