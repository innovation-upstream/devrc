 ...
 *
 * Prerequisites:
 *   - An account configured in the STATE dir (see lib/paths.mjs) — normally
 *     ~/.local/state/clickup/accounts.json, written by `query.mjs add-account`.
 *   - Live-workspace fixtures, each via env var OR a key on the default account.
 *     Any that is unset makes the tests needing it SKIP loudly, never pass:
 *       CLICKUP_SMOKE_LIST_ID       / smokeListId       — list to create into
 *       CLICKUP_SMOKE_SPACE_ID      / smokeSpaceId      — space for create-list
 *       CLICKUP_SMOKE_DOC_ID        / smokeDocId        — persistent doc
 *       CLICKUP_SMOKE_DOC_PAGE_ID   / smokeDocPageId    — page with comments
 *       CLICKUP_SMOKE_MENTION_USER  / smokeMentionUser  — @mention target
 */
...
// ─── Live-workspace fixtures ──────────────────────────────────────────
//
// Each of these names a REAL object in the workspace under test, and this file
// is PUBLIC — so none of them may be hardcoded. One resolver for all of them
// (env var, then a key on the default account, then null); every test that
// needs one SKIPS loudly rather than passing vacuously or failing confusingly.
//
// Configure by either route:
//   CLICKUP_SMOKE_LIST_ID=… node test/smoke-test.mjs
//   …or add "smokeListId" etc. to the default account in accounts.json
function fixture(envVar, configKey) {
  const fromEnv = (process.env[envVar] || '').trim();
  if (fromEnv) return fromEnv;
  try {
    const creds = getAccountCredentials(null);
    return String(creds?.[configKey] || '').trim() || null;
  } catch {
    return null; // no accounts.json — every live test would fail anyway
  }
}

function needs(envVar, configKey, what) {
  return `no ${what} configured — set ${envVar}=<id>, or add "${configKey}" ` +
    'to the default account in accounts.json';
}

const TEST_LIST_ID      = fixture('CLICKUP_SMOKE_LIST_ID', 'smokeListId');
const TEST_SPACE_ID     = fixture('CLICKUP_SMOKE_SPACE_ID', 'smokeSpaceId');
const PERSISTENT_DOC_ID = fixture('CLICKUP_SMOKE_DOC_ID', 'smokeDocId');
const DOC_COMMENT_PAGE  = fixture('CLICKUP_SMOKE_DOC_PAGE_ID', 'smokeDocPageId');
const MENTION_USER      = fixture('CLICKUP_SMOKE_MENTION_USER', 'smokeMentionUser');

const LIST_SKIP     = needs('CLICKUP_SMOKE_LIST_ID', 'smokeListId', 'test list');
const SPACE_SKIP    = needs('CLICKUP_SMOKE_SPACE_ID', 'smokeSpaceId', 'test space');
const DOC_SKIP      = needs('CLICKUP_SMOKE_DOC_ID', 'smokeDocId', 'test doc');
const DOC_PAGE_SKIP = needs('CLICKUP_SMOKE_DOC_PAGE_ID', 'smokeDocPageId', 'test doc page');
const MENTION_SKIP_REASON =
  needs('CLICKUP_SMOKE_MENTION_USER', 'smokeMentionUser', '@mention target');
...
async function readOnlyTests() {
  ...
  if (TEST_LIST_ID) {
    ...
    skip('tasks: lists tasks in a list', LIST_SKIP);
  }
  ...
  if (TEST_SPACE_ID) {
    ...
    skip('space-lists: lists folderless lists', SPACE_SKIP);
  }
...
async function writeTests() {
  ...
  // Every write test below creates into TEST_LIST_ID. Without it the `create`
  // test would fail with a confusing usage error and cascade; skip explicitly.
  if (!TEST_LIST_ID) {
    skip('Write tests', LIST_SKIP);
    return { testPageIds };
  }
  ...
  // NOT __dirname: the skill dir deploys read-only, so a scratch file there
  // fails with EROFS. These are throwaway fixtures, not state.
  const tmpFile = resolve(tmpdir(), 'clickup-smoke-test.md');
  const tmpFileCleanup = resolve(tmpdir(), 'clickup-smoke-cleanup.md');
  ...
  if (MENTION_USER) {
    await test('mention: posts comment with @[username] mention', async () => {
      const { stdout } = await runJson([
        'comment',
        testTaskId,
        `SMOKE TEST: Hey @[${MENTION_USER}], take a look`,
      ]);
      const data = assertJson(stdout);
      assert(data.id, 'Expected comment ID');
      // Clean up
    ...
    skip('mention: posts comment with @[username] mention', MENTION_SKIP_REASON);
  }
  ...
  if (!TEST_SPACE_ID) skip('create-list: creates a list', SPACE_SKIP);
  else await test('create-list: creates a list', async () => {
  ...
  if (!PERSISTENT_DOC_ID) {
    skip('Doc CRUD tests', DOC_SKIP);
    return { testPageIds };
  }
  ...
  // ── Doc Comments (Internal API, requires JWT) ──

  // DOC_COMMENT_PAGE is resolved at module load — a doc page with existing
  // comments in the workspace under test. Not hardcoded: this file is public.
  if (!DOC_COMMENT_PAGE) {
    skip('Doc comment tests', DOC_PAGE_SKIP);
    return { testPageIds };
  }

  let testDocCommentId = null;

  await test('doc-comments: lists page comments', async () => {
    const { output } = await run(['doc-comments', DOC_COMMENT_PAGE]);
    assertContains(output, 'comment(s)');
    assertContains(output, 'ID:');
  });

  await test('doc-comments --json: returns valid JSON', async () => {
    const { stdout } = await runJson(['doc-comments', DOC_COMMENT_PAGE]);
    const data = assertJson(stdout);
    assert(Array.isArray(data), 'Expected comments array');
    assert(data.length > 0, 'Expected at least one comment');
  });

  await test('doc-comment: posts new top-level comment', async () => {
    const { stdout } = await runJson(['doc-comment', DOC_COMMENT_PAGE, 'SMOKE TEST: Doc comment - delete me']);
    const data = assertJson(stdout);
    testDocCommentId = data.id?.toString();
    assert(testDocCommentId, 'Expected comment ID');
    if (VERBOSE) console.log(`    Created doc comment: ${testDocCommentId}`);
  });

  if (testDocCommentId) {
    await test('doc-reply: replies to a doc comment', async () => {
      const { stdout } = await runJson(['doc-reply', testDocCommentId, 'SMOKE TEST: Reply - delete me', '--page', DOC_COMMENT_PAGE]);
      const data = assertJson(stdout);
      assert(data.id, 'Expected reply ID');
      if (VERBOSE) console.log(`    Created doc reply: ${data.id}`);
    });

    await test('doc-comments: shows new comment in listing', async () => {
      const { output } = await run(['doc-comments', DOC_COMMENT_PAGE]);
      assertContains(output, 'SMOKE TEST: Doc comment');
    });
  }
