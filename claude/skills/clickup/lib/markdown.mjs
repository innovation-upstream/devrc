...
function processNode(node, comment, context) {
  switch (node.type) {
    ...
    case 'code':
      comment.push({ text: node.value, attributes: { 'code-block': { 'code-block': 'plain' } } });
      comment.push({ text: '\n', attributes: {} });
      break;
...
export function clickUpToMarkdown(commentArray) {
  ...
  for (let i = 0; i < commentArray.length; i++) {
    ...
    // Handle image/attachment elements
    if (item.type === 'image' && item.image) {
      const name = item.image.name || item.image.title || text || 'image';
      const url = item.image.url || '';
      if (url) {
        result += `[Attachment: ${name}](${url})`;
      } else {
    ...
    // Handle attachment elements (non-image files)
    if (item.type === 'attachment' && item.attachment) {
      const name = item.attachment.name || item.attachment.title || text || 'file';
    ...
    // Handle code blocks (both old boolean format and correct nested object format)
    if (attrs['code-block']) {
      result += '```\n' + text + '\n```';
