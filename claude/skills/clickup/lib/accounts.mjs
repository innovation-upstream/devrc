 ...
 *
 * accounts.json lives in the STATE dir ($XDG_STATE_HOME/clickup, else
 * ~/.local/state/clickup) — never in the skill dir, which is deployed
 * read-only. See lib/paths.mjs.
 *
 * Schema:
 * {
 *   "defaultAccount": "bot",
...
import { accountsPath, envPath, ensureStateDir, stateBaseDir } from './paths.mjs';
...
export function getAccountCredentials(accountId) {
  ...
  if (!config) {
    throw new Error(
      ...
      `Setup: Create accounts.json in ${stateBaseDir()}\n` +
      '  (or run: node query.mjs add-account <name> --token pk_...)\n' +
      'See reference/setup.md for details.'
    );
  }
