...
import { watchersFile, ensureStateDir } from '../lib/paths.mjs';

// ── Persistent watcher registry ───────────────────────────────────────────
// Lives in the state dir, not the skill dir (which deploys read-only).

function loadWatchers() {
  ensureStateDir();
  const file = watchersFile();
...
function saveWatchers(watchers) {
  ensureStateDir();
  const file = watchersFile();
  writeFileSync(file, JSON.stringify(watchers, null, 2));
  // Each entry carries the webhook's HMAC signing secret — keep it owner-only.
