...
import { readFileSync, existsSync, appendFileSync, chmodSync } from 'fs';
import { getAccountCredentials } from '../lib/accounts.mjs';
import { envPath, ensureStateDir, stateBaseDir } from '../lib/paths.mjs';

// The legacy .env lives in the STATE dir, not the skill dir (read-only once
// deployed). Re-exported as a function, not a const: the state dir is resolved
// lazily so XDG_STATE_HOME is honoured at call time.
export { envPath };
...
export function appendToEnv(key, value, comment = null) {
  try {
    ...
    try { chmodSync(env, 0o600); } catch { /* Windows may not support */ }
    process.env[key] = value;
    return true;
  } catch (e) {
    return false;
...
function getToken() {
  ...
  if (!token) {
    ...
    console.error('');
    console.error('Setup:');
    console.error(`  1. Create accounts.json in ${stateBaseDir()} (see reference/setup.md)`);
