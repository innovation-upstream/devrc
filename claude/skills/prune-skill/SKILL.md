...
- 🔴 **Check what governs THIS file before executing the auditor's number — 12 KB is a SKILL budget.** A file that is not a skill body may be under a different gate, or under none. 🔴 **An eviction SINK — `claude/RULES-ARCHIVE.md`, a `claudedocs/` doc — is ungated and demand-loaded (0 B until opened), and PRUNING IT DELETES THE NARRATIVE A PRIOR PRUNE PUT THERE.** Never prune a sink; §3's "bias aggressively toward EVICT/DEMOTE/DROP" does not apply to one. Where a gate exists it OWNS its numbers — read them there, never from a doc restating them. A "within budget" verdict (§1) stays authoritative.
...
- 🔴 **A push is not a saving — a skill body loads from the DEPLOYED copy, which may not be the clone.** Resolve it first: `readlink -f ~/.claude/skills/<name>/SKILL.md`. Terminating **inside the repo** (an `mkOutOfStoreSymlink`) ⇒ the clone IS live. Terminating in **`/nix/store`** (a `home.file` copy — the devrc default) ⇒ the clone is NOT live and a `home-manager switch` is required; re-measure the **resolved** path, never `wc -c` in the clone. For a plain clone the failure is staleness: after five pushed prunes one was 160 commits behind, still serving the 92,270 B body. Re-sync (`fetch && merge --ff-only`), then re-measure at the resolved path. Expect the ff-merge to refuse on other sessions' in-flight docs; classify each blocker before touching it — procedure in `reference/always-loaded-and-landing.md`.
