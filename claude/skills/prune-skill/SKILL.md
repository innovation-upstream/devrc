...
- 🔴 **Check what governs THIS file first — 12 KB is a SKILL budget, and `skill-audit.py` prints devrc's number for EVERY repo.** Another repo's gate may be a ratchet with an allowance (talos-infra gate 11), which passes bodies this tool calls RED and blocks ones it calls fine. The tool now says so when the file is outside devrc; believe that line, not the number.
- 🔴 **A low core-to-reference ratio is a tell that points at CANDIDATES, not a target** — its floor is shape-dependent (a tool-shaped skill routes ~13x; a method-shaped one may correctly sit near 2x). Rationale, measurements and history are demotable in any shape.
