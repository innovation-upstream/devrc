...
- **Mutation-test a guard before certifying it** — break the thing on purpose and watch the guard go red. **A green sweep is only a claim about the mutations you IMAGINED**: large sweeps have had blind spots only a *differently-constructed independent* sweep found — one **surviving** mutant silently reintroduced the very bug the PR fixed. Vary how the sweep is built, not just the mutant count; pick fixtures whose fields are pairwise distinct. → archive: mutation-sweep-blind-spots
...
The stash stack is shared across ALL worktrees of a repo, so a concurrent agent or session can pop *your* stash. 🔴 **The prohibition is NOT scoped to rebases, and applies for ANY reason** — a narrower earlier wording is exactly how it failed (the preamble's example): knowing the rule did not help, because "hazardous for rebases" is not "don't". → archive: stash-incidents
