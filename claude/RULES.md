...
- **Re-check WHICH branch you are on before ANY `pull`/`rebase`/`checkout` in a shared checkout** — other sessions switch branches under you, so the branch you started on is not the branch you are on now. Observed: a `git pull --rebase origin main` issued without re-checking rebased *another session's* feature branch onto main (content survived, base moved); separately another session's `git checkout` silently reverted a staged build mid-verification and deleted a test-fixture directory.
