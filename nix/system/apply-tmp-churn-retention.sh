...
_close = src.find("];", _open)
if _close == -1:
    raise SystemExit("ERROR: 'systemd.tmpfiles.rules = [' is never closed with '];'")
...
for _r in RULES:
    ...
    # `.*"` stops at the closing quote, so a trailing comment after it does NOT
    # defeat the match — that was a second defect: an upgrade left both an m:7d
    # and an mM:7d line for one path whenever the old line carried a comment.
    _pat = _re.compile(
...
for _e_line in list(_evicted):
    # re-find in the CURRENT block; replace one occurrence at a time so the count
    # is a count of REMOVALS, not of matches (two identical stale lines used to
    # report "evicted 2" while str.replace removed both on the first pass).
    _idx = _block.find(_e_line)
    if _idx != -1:
        _nl = _block.find("\n", _idx)
        _block = _block[:_idx] + _block[_nl + 1:] if _nl != -1 else _block[:_idx]
