...
// snapshot. Diff-first is caught by `diffBody`'s `Load == LoadReady` condition
// while the panel query is still out, and then by the `PRLoaded` issue arm,
// which clears what the speculative read left behind. A test that drove one
// order would leave the other's mechanism unexercised.
...
// 🔴 A LATE DIFF RESULT MUST NOT OVERWRITE THE PAGE'S OWN ERROR.
//
...
func TestAPageFailureKeepsItsOwnErrorOverALaterDiffFailure(t *testing.T) {
	...
	got := deliver(t, false, PRLoaded{Err: pageErr}, DiffLoaded{Err: diffErr})

	if got.Load != LoadFailed {
		t.Fatalf("Load = %v, want LoadFailed", got.Load)
	}
	screen := stripANSI(got.render())
	// The card carries the PAGE's state word and the PAGE's words.
	if !strings.Contains(screen, ghapi.AuthNotFound.Word()) {
		t.Errorf("the error card does not carry the PAGE's state word %q\n%s",
			ghapi.AuthNotFound.Word(), screen)
	}
	if !strings.Contains(screen, "the page leg's own words") {
		t.Errorf("the error card does not carry the PAGE's message\n%s", screen)
	}
	// And NOT the diff leg's.
	if strings.Contains(screen, ghapi.AuthRateLimited.Word()) {
		t.Errorf("the diff leg's state word %q reached the error card, which was "+
			"built from the PAGE's failure\n%s", ghapi.AuthRateLimited.Word(), screen)
	}
	if strings.Contains(screen, "the diff leg's own words") {
		t.Errorf("the diff leg's message reached the error card\n%s", screen)
	}
}
