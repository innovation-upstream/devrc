...
let
  ...
  # threshold that block set for extracting this helper, so both weak copies
  # now share it. The generated bash must stay BEHAVIOR-IDENTICAL to the
  # 2026-09-19 discord-embed script — verified byte-for-byte against the
  # deployed generation's activate script for the dee instance before
  # shipping this refactor.
  #
  # browser-bridge deliberately does NOT use this helper: its RENAME_EXCHANGE
  # two-attempt TOCTOU dance exists because a live server serves traffic
  # ACROSS the switch; this helper's weaker rename-away swap is for
  # extensions serving nothing mid-switch. Do not point browser-bridge here
  # without re-measuring that dance.
  #
  # `var` MUST be unique per instance: every activation entry's script body
  # runs in ONE bash process, so a shared prefix would collide. `extName`
  # names the extension in every message AND is the deployed directory name
  # ("$HOME/<dstDir>/<extName>"). `src` is the in-repo unpacked extension
  # tree (⚠ FLAKE TRAP, see the discord-embed comment: a new extension file
  # that is not `git add`ed is silently omitted from the store copy — no
  # error anywhere). `dstDir` is the parent directory under $HOME.
  mkUnpackedExtensionDeploy = { var, extName, src, dstDir }:
    lib.hm.dag.entryAfter ["writeBoundary"] ''
      ${var}Src=${src}
      ${var}Dst="$HOME/${dstDir}/${extName}"
      ${var}Tmp="''$${var}Dst.new.$$"
      $DRY_RUN_CMD mkdir -p "$HOME/${dstDir}"
      ...
      for ${var}Old in "''$${var}Dst".new.* "''$${var}Dst".old.*; do
        ...
        if [ -n "''$${var}Pid" ]; then
          ...
          [ -d "/proc/''$${var}Pid" ] && [ "''$${var}Pid" != "$$" ] && continue
        fi
        ${var}Scrub "''$${var}Old"
      done

      ${var}Scrub "''$${var}Tmp"                      # cp -rL NESTS if the target exists
      $DRY_RUN_CMD cp -rL "''$${var}Src" "''$${var}Tmp" # -L → real files, not store symlinks
      $DRY_RUN_CMD chmod -R u+rwX "''$${var}Tmp"   # writable for the NEXT switch's rm -rf

      if [ -n "''${DRY_RUN_CMD:-}" ]; then
        echo "would install the ${extName} extension at ''$${var}Dst:"
        echo "  mv -T ''$${var}Dst ''$${var}Dst.old.$$   # only if it already exists"
        echo "  mv -T ''$${var}Tmp ''$${var}Dst"
      else
        # Move the old tree aside BEFORE putting the new one in place, so the
        # window in which ''$${var}Dst does not exist is a rename, not a copy. A
        # failure here leaves the previous tree at .old.<pid> for the sweep
        # above to reclaim — never a half-written ''$${var}Dst.
        ${var}Stash=""
        if [ -e "''$${var}Dst" ] || [ -L "''$${var}Dst" ]; then
          ${var}Stash="''$${var}Dst.old.$$"
          ...
          ${var}Scrub "''$${var}Stash"
          if ! mv -T "''$${var}Dst" "''$${var}Stash"; then
            echo "${extName}: deploy FAILED — could not move the existing" \
                 "''$${var}Dst aside. Nothing was changed; the new tree is at ''$${var}Tmp." >&2
            false
          fi
        fi
        if ! mv -T "''$${var}Tmp" "''$${var}Dst"; then
          echo "${extName}: deploy FAILED — could not install ''$${var}Tmp at" \
               "''$${var}Dst." >&2
          if [ -n "''$${var}Stash" ] && mv -T "''$${var}Stash" "''$${var}Dst"; then
            echo "${extName}: RESTORED the previous extension tree." >&2
          else
            echo "${extName}: the previous tree is at ''$${var}Stash — Brave" \
...
in
{
    ...
    matches = {
      base = {
        matches = [
          ...
          { trigger = ":cobsd"; replace = "call opencode to use browser skill to drive"; label = "Call opencode to use browser skill to drive"; search_terms = ["opencode" "call" ]; }
  ...
  #
  # 🔴 WAS A SECOND COPY OF A PREDICATE, KNOWINGLY — the comment below used to
  # read "if a THIRD extension wants this, extract the helper first instead of
  # pasting again." claude-usage is the third, so the helper now exists (see
  # `mkUnpackedExtensionDeploy` in the let block) and this entry calls it. The
  # generated bash is verified byte-identical to this block's 2026-09-19
  # rendering; the weak copy is deliberately weaker than browser-bridge's: no
  # `--exchange` two-attempt TOCTOU dance, because nothing here is serving
  # traffic mid-switch — the failure it must avoid is a HALF-WRITTEN extension
  # directory, not a torn handoff between a live server and its client.
  #
  ...
  home.activation.discordEmbedExtension = mkUnpackedExtensionDeploy {
    var = "dee";
    extName = "discord-embed-ext";
    src = ../scripts/discord-embed-ext/extension;
    dstDir = ".local/share";
  };

  # claude-usage tracker: same git-immunity problem and same remedy — see the
  # browser-bridge and discord-embed comments above. Scope: claudedocs/
  # proposal-claude-usage-tracker.md (snapshots every Claude account's usage on
  # claude.ai open; toasts + popup). No sidecar: single-profile, in-product
  # account switching; chrome.storage carries all state.
  #
  # ⚠ FLAKE TRAP (see mkUnpackedExtensionDeploy): `git add` new extension files
  # BEFORE switching — an untracked file is silently omitted from the store
  # copy and therefore from the deployed tree, no error anywhere.
  #
  # MANUAL operator step, once: brave://extensions → Developer mode → Load
  # unpacked → ~/.local/share/claude-usage-ext. Nix cannot register an unpacked
  # extension with Brave; it can only keep the directory correct.
  home.activation.claudeUsageExtension = mkUnpackedExtensionDeploy {
    var = "cue";
    extName = "claude-usage-ext";
    src = ../scripts/claude-usage/extension;
    dstDir = ".local/share";
  };
  ...
  # `lib/cairn_pin.py` through its own `Path(__file__).resolve().parent / "lib"`,
  # and there IS NO PACKAGE shipping those beside it — `subsystem_touch.py` is the
  # devrc-only WRITER and is deliberately absent from the OSS repo, which
  # extracted the reader only, and `cairn_pin.py` is devrc's own seam onto the
  # pinned package. A `home.file` copy would resolve into /nix/store and die on
  # import before printing anything. So the asymmetry above is now
  # two-against-one: `cairn` is in-store because its package installs script and
  # `lib/` together; these two are out-of-store because nothing installs theirs.
  # Read all three lines together and do not "tidy" them into agreement in either
  # direction.
  #
  # ⚠ `subsystem_read_store` USED TO BE NAMED HERE and no longer is: devrc deleted
  # its forked copy when it consolidated onto the pin, so that module arrives
  # through `flake.lock`. The argument above is UNCHANGED by that — it was never
  # about the reader modules, it is about `subsystem_touch.py`, which no package
  ...
  systemd.user.services.task-spec-drafter = lib.mkIf serverMode {
    ...
    Service = {
      ...
      Environment = [
        ...
        "PATH=%h/.nix-profile/bin:/run/current-system/sw/bin:${lib.makeBinPath [ pkgs.nodejs_24 pkgs.git pkgs.kubectl pkgs.jq pkgs.curl pkgs.python312 pkgs.bash pkgs.coreutils pkgs.gnugrep pkgs.gnused pkgs.gawk ]}"
        "HOME=%h"
