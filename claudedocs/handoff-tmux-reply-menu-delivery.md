...
2. ~~**Cut a clawgate release so #855's scroll fix reaches the pod.**~~ ✅ **ALREADY LIVE — NO RELEASE CUT (2026-09-20, operator's call).** This item was **STALE WHEN WRITTEN AS RANK 2**: the pin had already moved to **0.8.46** and `edefd679f` is an ancestor of the pin-bump commit `b585023f8`. Verified **by CONTENT, not ancestry** — `docker create` + `export` of the running `clawgate:0.8.46`, then `LC_ALL=C command grep -ac`: the fix's rendered marker `snap-start snap-always` = **1**, positive control `AskUserQuestion` = **3**, negative control = **0**. Live pin, pod image and `clawgatectl health` all agree at 0.8.46. The whole `b585023f8..origin/trunk` diff under `containers/clawgate/` is **`HANDOFF.md` alone** (16+/8-), and the only open clawgate PR, **#838**, is explicitly `[HELD]`. So a 0.8.47 would have rebuilt a behaviourally identical image and restarted the pod to ship a docs edit.
   🔴 **Two traps this closed, both worth keeping.** (a) **A ranked item is a claim with an expiry** — this one was acted on a few hours later and the premise had already changed; re-derive rank N's premise before building, never relay it. (b) **`grep` returned a FALSE ZERO on the image tar**: the bare wrapper scored **0 for the positive control**, i.e. indistinguishable from "the fix is absent". `LC_ALL=C command grep -a` scored it 3. **A zero from this instrument is meaningless until a positive control moves it.**
