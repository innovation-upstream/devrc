...
## State now
- Base clone at `972fbcbd`, in sync with `origin/main` (it was 2 commits behind at session start and was fast-forwarded). This doc was landed from branch `docs/handoff-scratchpad-bar-dispatch` — devrc's CLAUDE.md forbids committing to `main` in either host checkout, so the handoff goes via PR like any other change.
