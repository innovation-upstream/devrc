...
- 🔴 **EVERY ZERO IS A POST-DECISION DISPATCH, AND THAT IS THE WHOLE FINDING.** `#1523` merged **27 min BEFORE** its audit was dispatched (156 min open-to-merge); `#1510` merged **+6 min AFTER** dispatch, inside its own audit's runtime; `#1518` was **closed 5 min BEFORE** trial 4 returned. Runtimes were 459 s / 920 s / 776 s, so no achievable speedup changes any of them — nothing fires round 0 at PR-open time, which is the only moment its question ("should this change exist") is actionable. **The section is fast enough; it is not TRIGGERED.** Verdicts: trial 3 `deletion candidate`, trial 4 `deletion candidate`, trial 5 `requirement questioned`.
- 🔴 **MY TRIAL 4 CERTIFIED A FALSE FIGURE, AND A PEER'S ROUND 0 CAUGHT IT FIVE MINUTES LATER.** Briefed to re-verify `#1518`'s self-reported "0 of 16" rather than accept it, the auditor wrote its own extractor *from the method description* — believing that made it independent — matched **13 of 13 figures** including a 36.1% positive control, and concluded the requirement HOLDS. It does not: `#1518` was closed unmerged 01:39:29Z, its closing comment crediting *"round 0 and round 1"* for a single counterexample, with corrected yields of **12 of 38 (32%) pre-fix and 9 of 17 (53%) post-fix**. Trial 4's other two findings collapsed with the PR.
- 🔴 **TRIAL 3'S FINDING IS LIVE ON `main` AND IN FLIGHT AS `#1547`** — `scripts/tests/test_diagnose_disk_accounting.sh:1731` asserts *"Both files carry the sentence"* about a rule only one file has ever carried. Re-verified 2026-09-12 against `origin/main`: `git log -S'reader can resolve'` → **0** commits for `scripts/diagnose-disk-accounting.sh`, 3 for the test file, with `grep -c sha` → **9** over 1,169 lines as the positive control. The rule is **self-attributed**. See rank 10 — **do not start it, `#1547` exists.**
- ⚠ **Trial 5's headline 🔴 was REFUTED by re-measuring it 15 minutes later**; its 🟡 survived and is rank 11. The vetr DMARC monitor is live on the laptop (timer next `09:41:50 CDT`, last run `Sep 11 20:38:03 [ok]`) and its timer is absent **by design** on the workbench, where the `builtins.pathExists` gate is false. What survives is that the unit is declarative while its script and credential are managed by nothing.
- **Round 0 itself verified live**: deployed `~/.claude/skills/audit-pr/SKILL.md` blob OID **identical** to `origin/main` (`a79300b76fd8376496028d81099cfe3b8278e1e4`); `--round 0` emits `## ROUND 0 — QUESTION THE REQUIREMENT, THEN DELETE` where `--round 1` emits `## AUDIT FOR` (paired control); `--round 0 --emit-claims` refuses **rc 4**. `audit-pr/SKILL.md` is **34,112 B**, not the "~27 KB" rank 4 has carried.
...
🔴 **Numbering PRESERVED from main's list — live `claim-work` refs are keyed to it (`audit-pr-ladder-1` is held). Identify an item by its SUBJECT, never its rank.** Ranks 10-11 are new.

1. ✅ **DONE — round-0 trials 3-5 ran. The pair is `ran: 6 · changed the outcome: 3`, so the section STAYS.** 🔴 **Transcribe that pair verbatim; do not recompute it loosely** — `audit-pr/SKILL.md` says *"REPORT THE PAIR … never `C` alone … those have opposite conclusions"*. Decomposition: trials 1-2 = 2/2 · my trials 3-5 = 3/0 · the peer's `#1518` round 0 = 1/1. **The remaining work is the TRIGGER, not more trials** — every zero is a post-decision dispatch, so implement either (a) a `gh pr create`-time dispatch or (b) a poller over PRs younger than 15 minutes, then delete the RETIREMENT CONDITION from `claude/skills/audit-pr/SKILL.md` recording the pair. **Do NOT run more ordinary trials — five is enough and all the zeros share one cause.** forcing: none
...
10. 🔴 **IN FLIGHT as `innovation-upstream/devrc#1547` — do NOT start this, and sweep `gh pr list --state open` first.** Fix the false attribution trial 3 found, live on `main`: `scripts/tests/test_diagnose_disk_accounting.sh:1731` claims both files carry a sentence only one has ever carried (re-verified twice with a positive control; the rule is self-attributed). Comment-only; the guard itself is sound. ⚠ **The PR's correction QUOTES the old wording**, so `grep -c 'Both files carry the sentence'` stays **1** after it merges — read the surrounding lines, never the count: the survivor must sit under *"An earlier version of this paragraph read"*. forcing: regression — a comment asserting coverage that does not exist, in a guard whose whole purpose is to stop exactly that
