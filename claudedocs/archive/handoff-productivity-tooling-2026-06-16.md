...
- **Analysis pipeline preserved** → `devrc/scripts/session-analysis/` (was ephemeral in /tmp):
  - `extract_user_msgs.py` — pulls genuinely-typed messages from `~/.claude/projects/**/*.jsonl` (filters sidechain/meta/task-notifications/system-reminders/command-stdout; dedups). Run on each host, combine. Produces ~6,028 records. ⚠ Its CLI changed 2026-09-25 (devrc #1870): the positional output path is gone — use `--jsonl -o <path>`, and prefer `--arc`/`--session`/`--ids-file` to scope it.
