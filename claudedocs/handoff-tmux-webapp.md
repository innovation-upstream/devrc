...
🔴 **CLOSED investigations are DEMOTED, not deleted — verbatim in `claudedocs/refs/tmux-webapp-closed-investigations.md`.** 21 blocks whose thread reached a verdict (the #566 e2e/ci thread, the #591/#592 mutation-evidence thread, the store-api fsync diagnosis, the card-517 delivery axis, and the rank 61/18 `TaskRunTimeout` pair) were moved there when this doc came within 63 B of its size ceiling. They are lessons, not status, which is why they were demoted and not dropped. ⚠ A `refs/` file is NOT indexed by `handoff_search` — search will not surface them; the path above is the only way in.
